...
typedef struct {
  ...
  // For normal maps it is sufficient to use the following simplified version of the bitangent which
  // is generated at pixel/vertex level. bitangent = fSign * cross(vN, tangent); Note that the
  // results are returned unindexed. It is possible to generate a new index list But
  ...
  // For normal maps it is sufficient to use the following simplified version of the bitangent which
  // is generated at pixel/vertex level. fSign = bIsOrientationPreserving ? 1.0f : (-1.0f);
