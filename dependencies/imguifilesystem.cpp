...
#	warning DIRENT_USES_UTF8_CHARS is deprecatedand has become the default. (IMGUIFILESYSTEM_USE_ASCII_SHORT_PATHS_ON_WINDOWS can be used to disable it).
...
#  if (                                                                                            \
    !defined(IMGUIFS_MEMORY_USES_CHARS_AS_BYTES) ||                                                \
    !defined(IMGUIFILESYSTEM_USE_ASCII_SHORT_PATHS_ON_WINDOWS))
  const int MAX_FILENAME_BYTES = FILENAME_MAX * 4; // Worst case: 4 bytes per char, but huge waste
                                                   // of memory [we SHOULD have used imguistring.h!]
...
/*
        // Never tested (I've just been told that cl.exe does not have strcasecmp: please search the
   web for other possible alternative implementations) inline static int strcasecmp( const char *s1,
   const char *s2 )  { return _stricmp(s1,s2);
...
namespace ImGuiFs
{
  ...
  // Definitions of some helper classes (String,Path,SortingHelper,Directory). Better not expose
  ...
  // LIMITATIONS:
  // -> paths with '/','\\','.' bytes (and possibly a few others) inside multibyte codepoints are
  not supported       (*)
  ...
  // the result might not be correct if there's some multibyte codepoint that includes that byte(s)
  (bytes include '/','\\','.').

  // (**) String::ToLower() deeply breaks any multibyte char.
  // They're currently used only in:
  // Path::GetExtension(...)
  // Directory::GetFiles(const string& path,const string& wantedExtensions,const string&
  unwantedExtensions)
  // That's because file extensions must be returned lowercase, so that e.g. ".PNG" and ".png" can
  be string matched (even on case sensitive file systems).
  */
  class String
  {
  protected:
  ...
  public:
    inline static void PushBack(FilenameStringVector& rv, const char* s)
    {
      if (rv.Size == rv.Capacity) {
        rv.reserve(
          rv._grow_capacity(rv.Size + 1)); // optional optimization from ImVector<>::push_back()
      }
    ...
    inline static void Substr(const char* text, char* rv, int start, int count = -1)
    {
      ...
      if (count < 0) {
        count = (int)strlen(text) - start;
      }
      if (count > 0) {
        strncpy(rv, &text[start], count);
      }
      rv[count] = '\0';
    }
    ...
    inline static void Replace(
      const char* baseText, const char textToReplace, const char replacement, char* rv)
    {
      strcpy(rv, baseText);
    ...
    inline static void wide_to_utf8(const wchar_t* wstr, char* rv)
    {
      ...
      // rv[size_needed]='\0';              // If the parameter after wstr is -1, the function
      // processes the entire input string, including the terminating null character. Therefore, the
      // resulting character string has a terminating null character, and the length returned by the
    ...
    inline static void utf8_to_wide(const char* str, wchar_t* rv)
    {
      ...
      // rv[size_needed]=L'\0';            // // If the parameter after str is -1, the function
      // processes the entire input string, including the terminating null character. Therefore, the
      // resulting character string has a terminating null character, and the length returned by the
      // function includes this character.
      return;
  ...
  public:
    static void GetAbsolutePath(const char* path, char* rv)
    {
      ...
      size_t len;
      while ((len = strlen(rv)) > 0 && rv[len - 1] == '/') {
        rv[len - 1] = '\0';
      }
    ...
    static inline bool HasZipExtension(const char* filePath)
    {
      ...
      static const char lower[] = ".zip";
      static const char upper[] = ".ZIP";
      char c;
      for (int i = 1; i < 4; i++) {
        c = p1[i];
    ...
    static void Combine(
      const char* directory, const char* fileName, char* rv, bool appendMode = true)
    {
      if (!appendMode) {
        rv[0] = '\0';
      }
    ...
    static void Append(const char* directory, char* rv)
    {
      ...
      if (size > 0 && (rv[size - 1] != '\\' && rv[size - 1] != '/')) {
        strcat(rv, "/");
        ++size;
      }
      strcat(rv, directory);
      size = strlen(rv);
      while (size > 0 && (rv[size - 1] == '\\' || rv[size - 1] == '/')) {
        rv[size - 1] = '\0';
        --size;
      }
      if (size == 0 || rv[size - 1] == ':') {
    ...
    static void Split(
      const char* path, FilenameStringVector& rv, bool leaveIntermediateTrailingSlashes = true)
    {
      rv.clear();
      ...
      if (rv.size() > 0 && strlen(rv[0]) == 0) {
        strcpy((char*)&rv[0][0], "/\0");
      }
#ifdef _WIN32
      if (rv.size() == 1 && strlen(rv[0]) > 0 && rv[0][strlen(rv[0]) - 1] == ':') {
        strcat((char*)&rv[0][0], "/");
      }
#endif //_WIN32
      return;
  ...
  (acceptLinks ? S_ISLNK(statbuf.st_mode) : 1));
          }
  protected:
          File() {}
  };
  */
  ...
  public:
    ...
    inline static const SorterSignature& SetSorter(Sorting sorting)
    {
      ...
      if (isort >= 0 && isort < (int)SORT_ORDER_COUNT) {
  ...
  protected:
    ...
    // Please note that calling stat(...) every time inside sorters is a suicide! And that I'm doing
    // right that! (but I guess and hope that on many systems the calls get cached somewhere:
    // otherwise it would take ages to sort)
    static int IMGUIFS_CDECL Lastmodsort(const struct dirent** e1, const struct dirent** e2)
    {
      if (stat((*e1)->d_name, &stat1) == -1) {
        return -1;
      }
      if (stat((*e2)->d_name, &stat2) == -1) {
        return 1;
      }
    ...
    // Please note that calculating the file extension every time inside sorters is a suicide (well,
  ...
  const SortingHelper::SorterSignature SortingHelper::Sorters[] = {
    &SortingHelper::Alphasort,
    &SortingHelper::Alphasortinverse,
    &SortingHelper::Lastmodsort,
    &SortingHelper::Lastmodsortinverse,
    &SortingHelper::Sizesort,
    &SortingHelper::Sizesortinverse,
    &SortingHelper::Typesort,
    &SortingHelper::Typesortinverse};
  ...
  public:
      ...
      Sorting sorting = SORT_ORDER_ALPHABETIC)
    {
      ...
      static char tempString[MAX_PATH_BYTES];
      size_t sz;
...
#endif //_WIN32
      const int n =
        scandir(directoryName2, &eps, DirentGetDirectories, SortingHelper::SetSorter(sorting));

      static char directoryNameWithoutSlash[MAX_PATH_BYTES];
      if (sz > 0 && directoryName[sz - 1] == '/') {
        String::Substr(directoryName, directoryNameWithoutSlash, 0, sz - 1);
      } else {
        strcpy(directoryNameWithoutSlash, directoryName);
      }

      if (n >= 0) {
        result.reserve((size_t)n);
        if (pOptionalNamesOut) {
          pOptionalNamesOut->reserve((size_t)n);
        }
        for (int cnt = 0; cnt < n; ++cnt) {
          ...
          if (sz > 0) {
            if (
              strcmp(pName, ".") != 0 && strcmp(pName, "..") != 0 && pName[0] != '.' &&
              pName[sz - 1] != '~'
              ...
              if (pOptionalNamesOut) {
                String::PushBack(*pOptionalNamesOut, pName);
              }
            }
          }
          free(eps[cnt]);
...
#endif //_WIN32
      const int n =
        scandir(directoryName2, &eps, DirentGetFiles, SortingHelper::SetSorter(sorting));
      ...
      Sorting sorting = SORT_ORDER_ALPHABETIC)
    {
      ...
      if (
        (wantedExtensions == 0 || strlen(wantedExtensions) == 0) &&
        (unwantedExtensions == 0 || strlen(unwantedExtensions) == 0)) {
        files = filesIn;
        return;
      }
      files.clear();
      ...
      char wext[MAX_PATH_BYTES];
      String::ToLower(wantedExtensions, wext);
      char woext[MAX_PATH_BYTES];
      String::ToLower(unwantedExtensions, woext);

      char ext[MAX_PATH_BYTES];
      if (wantedExtensions && strlen(wantedExtensions) > 0) {
        files.reserve(filesIn.size());
        if (pOptionalNamesOut) {
          pOptionalNamesOut->reserve(namesIn.size());
        }
        FilenameStringVector wExts;
        String::Split(wext, wExts, ';');
          ...
          for (size_t i = 0, sz = filesIn.size(); i < sz; i++) {
            ...
            for (size_t e = 0; e < wExtsSize; e++) {
              if (strcmp(ext, wExts[e]) == 0) {
                ...
                if (pOptionalNamesOut) {
                  String::PushBack(*pOptionalNamesOut, namesIn[i]);
                }
              }
            }
          }
        ...
        // files.reserve(filesIn.size());if (pOptionalNamesOut)
        // pOptionalNamesOut->reserve(namesIn.size());
        FilenameStringVector woExts;
        String::Split(woext, woExts, ';');
    ...
    inline static const PathStringVector& GetUserKnownDirectories(
      const FilenameStringVector** pOptionalUserKnownDirectoryDisplayNamesOut,
      ...
      bool forceUpdate = false)
    {
      ...
      if (pOptionalUserKnownDirectoryDisplayNamesOut) {
        *pOptionalUserKnownDirectoryDisplayNamesOut = &dn;
      }
      if (pOptionalNumberKnownUserDirectoriesExceptDrives) {
        *pOptionalNumberKnownUserDirectoriesExceptDrives = &numberKnownUserDirectoriesExceptDrives;
      }
      if (init && !forceUpdate) {
        return rv;
      }
      ...
      static const char* name[] = {
        "Desktop", "Documents", "Favorites", "Music", "Pictures", "Recent", "Video"};
      ...
      if (csidSize != nameSize) {
        fprintf(
          stderr,
          "ERROR in file: imguifilesystem.cpp. Directory::GetUserKnownDirectories(...) "
          "csidSize!=nameSize.\n");
      }
      char tmp[MAX_PATH_BYTES] = "C:/";
      while (tmp[0] <= 'Z') {
        if (Directory::Exists(tmp)) {
          String::PushBack(mediaFolders, tmp);
        }
        tmp[0] = (char)((int)tmp[0] + 1);
      }
      rv.reserve(csidSize + mediaFolders.size());
      dn.reserve(csidSize + mediaFolders.size());
      WCHAR path[MAX_PATH + 1];
      for (int i = 0; i < csidSize; i++) {
        if (!GetSpecialFolderPathW(csid[i], &path[0], NULL)) {
          continue;
        }
      ...
      static const bool ifHomeSubfolerIsFoundInMountLocationsForgetThatRootMountLocation =
        true; // That means: if "/media/myusername" exists, don't add any other "/media/..."
              // entries.
      char userMediaString[MAX_PATH_BYTES];
      bool lastGood = false;
      for (int mntLocIndex = 0, sz = 2 * mountLocationSize; mntLocIndex < sz; mntLocIndex++) {
        ...
        } else if (lastGood && ifHomeSubfolerIsFoundInMountLocationsForgetThatRootMountLocation) {
          lastGood = false;
          continue;
        } // see "ifHomeSubfolerIsFoundInMountLocationsForgetThatRootMountLocation" above
        else {
          strcpy(userMediaString, mntLocString);
        }
        ...
        dn.reserve(rv.size());
        for (int i = 0, msz = mediaFolders.size(); i < msz; i++) {
          if (strcmp(mediaFolders[i], temp) == 0) {
            continue; // I we have processed "/media/myusername" once cycle before, exclude it from
                      // processing "/media" subfolders
          }
...
#ifdef _WIN32
    static bool GetSpecialFolderPathW(
...
#  ifdef IMGUI_USE_MINIZIP
  struct UnZipFileImpl {
    ...
    struct unz_file_info64_plus {
      ...
      static void Sort(ImVector<unz_file_info64_plus>& fileInfos, Sorting sorting)
      {
        ...
        case SORT_ORDER_LAST_MODIFICATION_INVERSE:
          sorter = &unz_file_info64_plus::LastModsortinverse;
          break;
        ...
        if (sorter) {
          qsort(&fileInfos[0], fileInfos.size(), sizeof(unz_file_info64_plus), sorter);
        }
      }
      ...
      {
        return strcasecmp(
          static_cast<const unz_file_info64_plus*>(e1)->name,
          static_cast<const unz_file_info64_plus*>(e2)->name);
      }
      ...
      {
        const ZPOS64_T& size1 =
          static_cast<const unz_file_info64_plus*>(e1)->info.uncompressed_size;
        const ZPOS64_T& size2 =
          static_cast<const unz_file_info64_plus*>(e2)->info.uncompressed_size;
      ...
      inline static int IMGUIFS_CDECL Typesort(const void* e1, const void* e2)
      {
        static const int dot = (int)'.';
        const char* p1 =
          strrchr((const char*)static_cast<const unz_file_info64_plus*>(e1)->name, dot);
        const char* p2 =
          strrchr((const char*)static_cast<const unz_file_info64_plus*>(e2)->name, dot);
        if (!p1) {
          return (!p2 ? 0 : -1);
        } else if (!p2) {
          return 1;
    ...
    bool getFilesOrDirectories(
      bool fileMode,
      const char* directoryName,
      PathStringVector& result,
      FilenameStringVector* pOptionalNamesOut,
      Sorting sorting,
      bool prefixResultWithTheFullPathOfTheZipFile) const
    {
      result.clear();
      if (pOptionalNamesOut) {
        pOptionalNamesOut->clear();
      ...
      if (directoryName) {
        ...
        if (
          dirNameLen > 1 && directoryName[0] == '.' &&
          (directoryName[1] == '/' || directoryName[1] == '\\')) {
          strcpy(dirName, &directoryName[2]);
        } else {
          strcpy(dirName, directoryName);
        }
      } else {
        strcpy(dirName, directoryName ? directoryName : "");
      }
      dirNameLen = strlen(dirName);
      while (dirNameLen > 0 &&
             (dirName[dirNameLen - 1] == '/' || dirName[dirNameLen - 1] == '\\')) {
      ...
      // Fill fileInfos
      char tmp[MAX_PATH_BYTES] = "";
      char filename_inzip[MAX_PATH_BYTES] = "";
      const char charCrypt[2] = "*";
      ...
      for (i = 0; i < gi.number_entry; i++) {
        err = unzGetCurrentFileInfo64(
          uf, &file_info, filename_inzip, sizeof(filename_inzip), NULL, 0, NULL, 0);
        if (err != UNZ_OK) {
        ...
        const bool isDirectory =
          filename_inzip_len > 0 && (filename_inzip[filename_inzip_len - 1] == '/' ||
                                     filename_inzip[filename_inzip_len - 1] == '\\');
        ok = fileMode                       ? (!isDirectory && !hasZeroSize)
             : (isDirectory && hasZeroSize) ? true
                                            : false;

        if (ok) {
          if (dirNameLen > 0) {
            if (dirNameLen >= filename_inzip_len) {
              ok = false;
            }
            if (strncmp(dirName, filename_inzip, dirNameLen) != 0) {
              ok = false;
            }
            if (filename_inzip[dirNameLen] != '/' && filename_inzip[dirNameLen] != '\\') {
              ok = false;
            }
            if (!fileMode && dirNameLen + 1 == filename_inzip_len) {
              ok = false;
            }
          }
          if (ok) {
            ...
            if (
              filename_inzip_len > 0 && (filename_inzip[filename_inzip_len - 1] == '/' ||
                                         filename_inzip[filename_inzip_len - 1] == '\\')) {
              filename_inzip[filename_inzip_len - 1] = '\0';
              --filename_inzip_len;
            } // remove trailing dash

            strcpy(
              tmp,
              dirNameLen > 0 ? &filename_inzip[dirNameLen + 1]
                             : filename_inzip); // Here "tmp" if the plain name, and
                                                // "filename_inzip" the full name
            ...
            if (ok) {
              if ((file_info.flag & 1) != 0) {
                strcat(
                  tmp, charCrypt); // Optional display a '*' in the file name only if it is crypted
              }
      ...
      if (fileInfos.size() > 0) {
        // sorting:
        if (!fileMode && sorting >= SORT_ORDER_SIZE) {
      ...
      /*
       if ((file_info.flag & 1) != 0) strcat(tmp,charCrypt);
                                              ...
                                              if (pOptionalNamesOut)  {
                                                      pOptionalNamesOut->resize(resultSize+1);
                                                      char* pName =
       &(*pOptionalNamesOut)[resultSize][0]; strcpy(pName,tmp); if ((file_info.flag & 1) != 0)
       strcat(pName,charCrypt); // display a '*' if the file is crypted
    ...
    static bool PathSplitFirstZipFolder(
      const char* path, char* rv1, char* rv2, bool rv1IsAbsolutePath = true)
    {
      rv1[0] = '\0';
      rv2[0] = '\0';
      if (path) {
        int gc = 0;
        const char* lowerZip = ".zip";
        const char* upperZip = ".ZIP";
  ...
  {
    if (!reloadIfAlreadyLoaded && isValid() && strcmp(zipFilePath, im->zipFilePath) == 0) {
      return true;
    }
    close();
    if (zipFilePath) {
  ...
  const char* UnZipFile::getZipFilePath() const { return im->zipFilePath; }
  bool UnZipFile::isValid() const { return im->uf != NULL; }
  ...
  bool UnZipFile::getDirectories(
    ...
    bool prefixResultWithTheFullPathOfTheZipFile) const
  {
    return im->getFilesOrDirectories(
      false,
      directoryName,
      result,
      pOptionalNamesOut,
      sorting,
      prefixResultWithTheFullPathOfTheZipFile);
  }
  bool UnZipFile::getFiles(
    const char* directoryName,
    PathStringVector& result,
    FilenameStringVector* pOptionalNamesOut,
    Sorting sorting,
  ...
  unsigned int UnZipFile::getFileSize(const char* filePath) const
  {
    ...
    if (unzLocateFile(im->uf, filePath, 0) != UNZ_OK) {
      return 0;
    }
    unz_file_info64 file_info;
    char filename_inzip[2048];
    int err = unzGetCurrentFileInfo64(
      im->uf, &file_info, filename_inzip, sizeof(filename_inzip), NULL, 0, NULL, 0);
    if (err != UNZ_OK) {
      return 0;
    }
  ...
  static bool UnZipFileGetFileContentBase(
    ImGuiFs::UnZipFileImpl* im,
    const char* filePath,
  ...
  bool UnZipFile::getFileContent(
    const char* filePath, ImVector<unsigned char>& bufferOut, const char* password) const
  {
    return UnZipFileGetFileContentBase<unsigned char>(im, filePath, bufferOut, password);
  }
  bool UnZipFile::getFileContent(
    const char* filePath, ImVector<char>& bufferOut, const char* password) const
  {
    return UnZipFileGetFileContentBase<char>(im, filePath, bufferOut, password);
  }
  bool UnZipFile::exists(
    const char* pathInsideZip, bool reportOnlyFiles, bool reportOnlyDirectories) const
  {
    if (!im->uf || !pathInsideZip) {
      return false;
    }
    char path[MAX_PATH_BYTES];
    ...
    if (pathInsideZip) {
      ...
      if (
        dirNameLen > 1 && pathInsideZip[0] == '.' &&
        (pathInsideZip[1] == '/' || pathInsideZip[1] == '\\')) {
        strcpy(path, &pathInsideZip[2]);
      } else {
        strcpy(path, pathInsideZip);
      }
    } else {
      strcpy(path, pathInsideZip ? pathInsideZip : "");
    }

    dirNameLen = strlen(path);
    while (dirNameLen > 0 && (path[dirNameLen - 1] == '/' || path[dirNameLen - 1] == '\\')) {
      path[dirNameLen - 1] = '\0';
      --dirNameLen;
    }
    ...
    if (strlen(path) == 0 && !onlyFiles) {
      return true; // base zip folder
    }
    ...
    if (found && (onlyDirs || onlyFiles)) {
      ...
      if (err != UNZ_OK) {
        ...
        const bool isDirectory =
          filename_inzip_len > 0 && (filename_inzip[filename_inzip_len - 1] == '/' ||
                                     filename_inzip[filename_inzip_len - 1] == '\\');
        found = onlyFiles  ? (!isDirectory && !hasZeroSize)
                : onlyDirs ? (isDirectory && hasZeroSize)
                           : false;
      }
    }
    unzCloseCurrentFile(im->uf);
  ...
  {
    return UnZipFileImpl::PathExistsWithZipSupport(
      path, reportOnlyFiles, reportOnlyDirectories, checkAbsolutePath, isInsideAZipFile);
  }
  ...
  bool DirectoryGetDirectoriesWithZipSupport(
    const char* directoryName,
    PathStringVector& result,
    FilenameStringVector* pOptionalNamesOut,
    Sorting sorting,
    bool prefixResultWithTheFullPathOfTheZipFile)
  {
    ...
    if (PathSplitFirstZipFolder(directoryName, zipRoot, zipFolder)) {
      ...
      return uzf.getDirectories(
        zipFolder, result, pOptionalNamesOut, sorting, prefixResultWithTheFullPathOfTheZipFile);
    }
    ImGuiFs::DirectoryGetDirectories(directoryName, result, pOptionalNamesOut, sorting);
    return true;
  }
  bool DirectoryGetFilesWithZipSupport(
    const char* directoryName,
    PathStringVector& result,
    FilenameStringVector* pOptionalNamesOut,
    Sorting sorting,
  ...
  void PathGetDirectoryNameWithZipSupport(
    const char* path, char* rv, bool prefixResultWithTheFullPathOfTheZipFile)
  {
    char zipRoot[ImGuiFs::MAX_PATH_BYTES] = "";
    ...
    if (PathSplitFirstZipFolder(path, zipRoot, zipRelativePath)) {
      ...
      ImGuiFs::PathGetDirectoryName(path, rv);
    }
  }
  void PathGetAbsoluteWithZipSupport(const char* path, char* rv)
  {
    ...
    if (PathSplitFirstZipFolder(path, zipRoot, zipRelativePath)) {
      ...
      strcpy(rv, tmp);
      if (strlen(zipRelativePath) > 0 && zipRelativePath[0] != '/' && zipRelativePath[0] != '\\') {
        strcat(rv, "/");
      }
      strcat(rv, zipRelativePath);
    } else {
      ImGuiFs::PathGetAbsolute(path, rv);
    }
  }
...
#  endif // IMGUI_USE_MINIZIP
  ...
  bool FileGetContentBase(
    const char* path, ImVector<CharType>& bufferOut, bool openInTextMode, const char* password)
  {
    bufferOut.clear();
...
#  endif // IMGUI_USE_MINIZIP
    ...
    if (
      (sizeof(long) > sizeof(size_t) && szl != (long)sz) ||
      (sizeof(long) < sizeof(size_t) && (size_t)szl != sz)) {
    ...
    if (sz > 0) {
      if (bufferOut.size() >= (int)sz) {
        fread(&bufferOut[0], (size_t)sz, 1, fin);
      } else {
        fprintf(stderr, "Error in: FileGetContent(\"%s\"): file too big.\n", mainPath);
        bufferOut.clear();
        fclose(fin);
  ...
  bool FileGetContent(
    const char* path, ImVector<unsigned char>& bufferOut, bool openInTextMode, const char* password)
  {
    return FileGetContentBase<unsigned char>(path, bufferOut, openInTextMode, password);
  }
  bool FileGetContent(
    const char* path, ImVector<char>& bufferOut, bool openInTextMode, const char* password)
  {
    return FileGetContentBase<char>(path, bufferOut, openInTextMode, password);
  }
#endif // IMGUIFS_NO_EXTRA_METHODS
       // End definitions of some helper
       // classes----------------------------------------------------------------------------------------
  ...
  struct ImGuiFsDrawIconStruct {
    ...
    int add(const char* exts)
    {
      ...
      int cnt = 0;
      const int vStartersCountValue =
        vStartersCounts.size() > 0 ? (vStartersCounts[vStartersCounts.size() - 1] + 1) : 0;
      static char tmp[512];
    ...
    {
      v.reserve(1024);
      vStarters.reserve(400);
      vStartersLengths.reserve(400);
    ...
    int getExtensionType(const char* ext, bool caseSensitiveMatch = false) const
    {
      ...
      for (int si = 0, siSz = vStarters.size(); si < siSz; si++) {
        ...
        start = vStarters[si];
        if (myStrCmp(&v[start], ext) == 0) {
          return vStartersCounts[si];
        }
      }
      return -1;
    }
    inline void fillExtensionTypesFromFilenames(
      ImVector<int>& fileExtensionTypes, const FilenameStringVector& fileNames)
    {
      fileExtensionTypes.resize(fileNames.size());
      for (int i = 0, isz = fileNames.size(); i < isz; i++) {
        fileExtensionTypes[i] = getExtensionType(strrchr(fileNames[i], '.'));
      }
    }

    inline bool drawIcon(int extensionType, const ImVec4* pOptionalColorOverride = NULL) const
    {
      return ImGuiFs::Dialog::DrawFileIconCallback
               ? ImGuiFs::Dialog::DrawFileIconCallback(extensionType, pOptionalColorOverride)
               : false;
    }
    inline bool drawIcon(
      const char* ext,
      bool caseSensitiveMatch = false,
      const ImVec4* pOptionalColorOverride = NULL) const
    {
    ...
    {
      static ImGuiFsDrawIconStruct instance;
      return instance;
    }
  };
  ...
  void FileGetExtensionTypesFromFilenames(
    ImVector<int>& fileExtensionTypesOut, const FilenameStringVector& fileNames)
  {
...
#  if (defined(__EMSCRIPTEN__) && defined(EMSCRIPTEN_SAVE_SHELL))
  bool FileDownload(const char* path, const char* optionalSaveFileName)
  {
    ...
    if (optionalSaveFileName) {
      buffer.appendf("saveFileFromMemoryFSToDisk(\"%s\",\"%s\")", path, optionalSaveFileName);
    } else {
...
#  endif // (defined(__EMSCRIPTEN__) && defined(EMSCRIPTEN_SAVE_SHELL))
  ...
  struct FolderInfo {
    ...
    void display() const
    {
      fprintf(
        stderr,
        "fullFolder=\"%s\" currentFolder=\"%s\" splitPathIndex=%d splitPathIndexOfZipFile=%d\n",
        fullFolder,
        currentFolder,
        splitPathIndex,
        splitPathIndexOfZipFile);
    }
    void getSplitPath(FilenameStringVector& splitPath) const { Path::Split(fullFolder, splitPath); }
...
#ifdef IMGUI_USE_MINIZIP
      const char* lower = ".zip";
      const char* upper = ".ZIP";
    ...
    {
      if (!path || strncmp(path, fullFolder, strlen(path)) != 0) {
        return -1;
      }
      ...
      // if (pOptionalSplitPathIndexOfZipFile) *pOptionalSplitPathIndexOfZipFile =
      // GetSplitPathIndexOfZipFile(SplitPath);
  ...
  public:
    inline bool canGoBack() { return currentInfoIndex > 0; }
    ...
    {
      if (!currentFolder || strlen(currentFolder) == 0) {
        return false;
      }
    ...
    {
      if (/*!fi.currentFolder ||*/ strlen(fi.currentFolder) == 0) {
        return false;
      }
      if (currentInfoIndex >= 0) {
        const FolderInfo& lastInfo = info[currentInfoIndex];
        if (lastInfo.isEqual(fi)) {
          return false;
        }
      }
      ++currentInfoIndex;
    ...
    {
      return (currentInfoIndex >= 0 && currentInfoIndex < (int)info.size());
    }
  ...
  struct Internal {
    ...
    inline static void FreeMemory(PathStringVector& v)
    {
      PathStringVector o;
      v.swap(o);
    }
#if FILENAME_MAX != DIRENT_MAX_PATH // otherwise PathStringVector == FilenameStringVector
    inline static void FreeMemory(FilenameStringVector& v)
    {
      FilenameStringVector o;
      v.swap(o);
    }
    ...
    inline void calculateBrowsingDataTableSizes(const ImVec2& childWindowSize = ImVec2(-1, -1))
    {
      ...
      if (childWindowSize.y > 0) {
        ...
        // static int tmp = 0;if (tmp!=numLinesThatFit)
        // {tmp=numLinesThatFit;fprintf(stderr,"childWindowSize.y = %f
    ...
    bool dontFilterSaveFilePathsEnteredByTheUser)
  {
    ...
    internal->mustFilterSaveFilePathWithFileFilterExtensionString =
      !dontFilterSaveFilePathsEnteredByTheUser;
  }
  Dialog::~Dialog()
  {
    if (internal) {
    ...
    const float windowAlpha)
  {
    ...
    bool& allowDirectoryCreation = I.allowDirectoryCreation =
      I.forbidDirectoryCreation ? false : (isSelectFolderDialog || isSaveFileDialog);
    //----------------------------------------------------------
    ...
    static const PathStringVector* pUserKnownDirectories = &Directory::GetUserKnownDirectories(
      &pUserKnownDirectoryDisplayNames, &pNumberKnownUserDirectoriesExceptDrives);
    //----------------------------------------------------------
    ...
    // Fill ColorSet above and fix dummyButtonColor here
    {
      ...
      for (int i = (int)Internal::ImGuiCol_Dialog_File_Background,
               sz = (int)Internal::ImGuiCol_Dialog_File_Text;
           i <= sz;
           i++) {
        ImVec4& c = ColorSet[i];
        const ImVec4& r =
          style.Colors
            [i < sz ? ((int)ImGuiCol_Button - (int)Internal::ImGuiCol_Dialog_File_Background + i)
                    : ImGuiCol_Text];
      ...
      for (int i = (int)Internal::ImGuiCol_Dialog_ZipDirectory_Background,
               sz = (int)Internal::ImGuiCol_Dialog_ZipDirectory_Text;
           i <= sz;
           i++) {
        ImVec4& c = ColorSet[i];
        const ImVec4& r = style.Colors[i < sz ? ((int)ImGuiCol_Button + i) : ImGuiCol_Text];
      ...
      if (dummyButtonColor.w > 0) {
        ...
        dummyButtonColor.x = bbc.x;
        dummyButtonColor.y = bbc.y;
        dummyButtonColor.z = bbc.z;
        dummyButtonColor.w *= bbc.w;
      }
      if (dummyZipButtonColor.w > 0) {
        const ImVec4& bbc = ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Background];
        dummyZipButtonColor.x = bbc.x;
        dummyZipButtonColor.y = bbc.y;
        dummyZipButtonColor.z = bbc.z;
        dummyZipButtonColor.w *= bbc.w;
      }
    }

    if (I.rescan) {
      char validDirectory[MAX_PATH_BYTES];
      validDirectory[0] = '\0'; // for robustness
#ifndef IMGUI_USE_MINIZIP
      if (directory && strlen(directory) > 0) {
        if (Directory::Exists(directory)) {
          strcpy(validDirectory, directory);
        } else {
...
#else  // IMGUI_USE_MINIZIP
      char basePath[MAX_PATH_BYTES];
      basePath[0] = '\0';
      char zipPath[MAX_PATH_BYTES];
      zipPath[0] = '\0';
      bool isInsideZipFile = false;
      if (directory && strlen(directory) > 0) {
        isInsideZipFile = PathSplitFirstZipFolder(directory, basePath, zipPath);
      } else {
        isInsideZipFile = PathSplitFirstZipFolder(I.currentFolder, basePath, zipPath);
      }
      if (isInsideZipFile ? FileExists(basePath) : Directory::Exists(basePath)) {
        strcpy(validDirectory, basePath);
        if (zipPath[0] != '\0') {
          char zipPathDir[MAX_PATH_BYTES];
          zipPathDir[0] = '\0';
          Path::GetDirectoryName(zipPath, zipPathDir);
          Path::Append(zipPathDir, validDirectory);
        }
        // fprintf(stderr,"basePath=%s zipPath=%s
      ...
      I.dirNames.clear();
      I.fileNames.clear();
      I.currentSplitPath.clear();
      I.fileExtensionTypes.clear();
      ...
      allowDirectoryCreation =
        I.forbidDirectoryCreation ? false : (isSelectFolderDialog || isSaveFileDialog);
      if (isSelectFolderDialog && I.sortingMode > SORT_ORDER_LAST_MODIFICATION_INVERSE) {
        I.sortingMode = 0;
      }
      I.forceRescan = true;
      I.open = true;
      ...
      if (!windowTitle || strlen(windowTitle) == 0) {
        if (isSelectFolderDialog) {
          strcpy(I.wndTitle, "Please select a folder");
        } else if (isSaveFileDialog) {
          strcpy(I.wndTitle, "Please choose/create a file for saving");
        } else {
          strcpy(I.wndTitle, "Please choose a file");
        }
      } else {
        strcpy(I.wndTitle, windowTitle);
      }
      strcat(I.wndTitle, "##");
      ...
      if (I.wndSize.x <= 0) {
        I.wndSize.x = Dialog::WindowSize.x;
      }
      if (I.wndSize.y <= 0) {
        I.wndSize.y = Dialog::WindowSize.y;
      }
      ...
      if (I.wndPos.x > screenSize.x - I.wndSize.x - Dialog::WindowLTRBOffsets.z) {
        I.wndPos.x = screenSize.x - I.wndSize.x - Dialog::WindowLTRBOffsets.z;
      }
      if (I.wndPos.y > screenSize.y - I.wndSize.y - Dialog::WindowLTRBOffsets.w) {
        I.wndPos.y = screenSize.y - I.wndSize.y - Dialog::WindowLTRBOffsets.w;
      }
      if (I.wndPos.x < Dialog::WindowLTRBOffsets.x) {
        I.wndPos.x = Dialog::WindowLTRBOffsets.x;
      }
      if (I.wndPos.y < Dialog::WindowLTRBOffsets.y) {
        I.wndPos.y = Dialog::WindowLTRBOffsets.y;
      }
      // fprintf(stderr,"screenSize = %f,%f mousePos = %f,%f wndPos = %f,%f wndSize =
      ...
      if (I.detectKnownDirectoriesAtEveryOpening) {
        pUserKnownDirectories = &Directory::GetUserKnownDirectories(
          &pUserKnownDirectoryDisplayNames, &pNumberKnownUserDirectoriesExceptDrives, true);
      }

      // fprintf(stderr,"Pos: %1.0f %1.0f Size: %1.0f
    ...
    if (I.forceRescan) {
      I.forceRescan = false;
#ifndef IMGUI_USE_MINIZIP
      const int sortingModeForDirectories =
        (I.sortingMode <= (int)SORT_ORDER_LAST_MODIFICATION_INVERSE) ? I.sortingMode
                                                                     : (I.sortingMode % 2);
      Directory::GetDirectories(
        I.currentFolder,
        I.dirs,
        &I.dirNames,
        (Sorting)sortingModeForDirectories); // this is because directories don't return their size
                                             // or their file extensions (so if needed we sort them
                                             // alphabetically)

      if (!isSelectFolderDialog) {
        if (!fileFilterExtensionString || strlen(fileFilterExtensionString) == 0) {
          Directory::GetFiles(I.currentFolder, I.files, &I.fileNames, (Sorting)I.sortingMode);
        } else {
          Directory::GetFiles(
            I.currentFolder,
            I.files,
        ...
        if (Dialog::DrawFileIconCallback) {
          ImGuiFsDrawIconStruct::Get().fillExtensionTypesFromFilenames(
            I.fileExtensionTypes, I.fileNames);
        }
      } else {
        I.files.clear();
        I.fileNames.clear();
      ...
      if (!isInsideZipFile) {
        Directory::GetDirectories(
          ...
          I.dirs,
          &I.dirNames,
          (Sorting)sortingModeForDirectories); // this is because directories don't return their
                                               // size or their file extensions (so if needed we
                                               // sort them alphabetically)
      } else {
        if (I.unz.load(basePath, false)) {
          I.unz.getDirectories(
            zipPath, I.dirs, &I.dirNames, (Sorting)sortingModeForDirectories, true);
        } else {
          I.unz.close();
        }
      }
      ...
      if (!isSelectFolderDialog) {
        if (!isInsideZipFile) {
          ...
          /*if (!fileFilterExtensionString || strlen(fileFilterExtensionString)==0)*/ I.unz
            .getFiles(zipPath, I.files, &I.fileNames, (Sorting)I.sortingMode, true);
          // else
          // I.unz.getFiles(zipPath,I.files,fileFilterExtensionString,NULL,&I.fileNames,(Sorting)I.sortingMode,true);
        }
      } else {
        I.files.clear();
        I.fileNames.clear();
      ...
      if (I.history.getInfoSize() > 0) {
        fprintf(
          stderr,
          "\nHISTORY: currentFolder:\"%s\" history.canGoBack=%s history.canGoForward=%s "
          "currentHistory:\n",
          I.currentFolder,
          I.history.canGoBack() ? "true" : "false",
          I.history.canGoForward() ? "true" : "false");
      }
...
#  undef DEBUG_HISTOTY
    ...
    const bool isBrowsingInsideZipFile =
      (fi &&
       (!(fi->splitPathIndexOfZipFile < 0 || fi->splitPathIndexOfZipFile > fi->splitPathIndex)));
    ...
    bool editLocationInputTextReturnPressed = false;
    {
      ...
      ImGui::PushStyleColor(
        ImGuiCol_Button,
        I.editLocationCheckButtonPressed ? dummyButtonColor : style.Colors[ImGuiCol_Button]);
      ...
      if (ImGui::Button("L##EditLocationCheckButton")) {
        ...
        // if (!I.editLocationCheckButtonPressed) mustValidateInputPath = true;   // or not ? I
        // mean: the user wants to quit or to validate in this case ?
      }
      ...
      if (I.editLocationCheckButtonPressed) {
        ImGui::SameLine();
        editLocationInputTextReturnPressed = ImGui::InputText(
      ...
      if (mustValidateInputPath) {
        ...
        while (len > 0 && (cleanEnteredPath[len - 1] == '/' || cleanEnteredPath[len - 1] == '\\')) {
          cleanEnteredPath[len - 1] = '\0';
          len = strlen(cleanEnteredPath);
        }

        if (len == 0 || strcmp(I.currentFolder, cleanEnteredPath) == 0) {
          I.editLocationCheckButtonPressed = false;
        }
#ifndef IMGUI_USE_MINIZIP
          ...
          if (!isInsideZipFile) {
            ...
            if (strcmp(I.unz.getZipFilePath(), basePath) == 0) {
              dirExists = I.unz.directoryExists(zipPath);
            } else {
...
#endif // IMGUI_USE_MINIZIP
       // else fprintf(stderr,"mustValidateInputPath NOOP: \"%s\"
      ...
      // Tab:
      {
        ...
        //-----------------------------------------------------
        {
          ...
          if (fi.splitPathIndexOfZipFile!=tmpzfi) {
                  ...
                  if (fi.splitPathIndexOfZipFile!=-1) fprintf(stderr,"%d
          %s\n",fi.splitPathIndexOfZipFile,I.currentSplitPath[fi.splitPathIndexOfZipFile]); else
          fprintf(stderr,"%d\n",fi.splitPathIndexOfZipFile);
          }*/
          //-------------------
          ...
          for (int t = 0; t < numTabs; t++) {
            if (t == fi.splitPathIndex) {
...
#ifdef IMGUI_USE_MINIZIP
              if (t == fi.splitPathIndexOfZipFile) {
                pDummyButtonColor = &dummyZipButtonColor;
              }
#endif // IMGUI_USE_MINIZIP
              ...
              ImGui::PushStyleColor(
                ImGuiCol_Button, ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Background]);
              ImGui::PushStyleColor(
                ImGuiCol_ButtonHovered, ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Hover]);
              ImGui::PushStyleColor(
                ImGuiCol_ButtonActive, ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Pressed]);
            }
#endif // IMGUI_USE_MINIZIP
#ifndef _MSC_VER
            ...
            if (wrapMode && sumX == 0) {
              sumX = ImGui::GetStyle().WindowPadding.x + ImGui::GetItemRectSize().x;
            }
            ImGui::PopID();
            if (pressed) {
    ...
    if (I.allowKnownDirectoriesSection && pUserKnownDirectories->size() > 0) {
      ...
      if (ImGui::CollapsingHeader("Known Directories##imguifs_UserKnownDirectories")) {
        ...
        ImGui::PushStyleColor(
          ImGuiCol_Button, ColorSet[Internal::ImGuiCol_Dialog_Directory_Background]);
        ImGui::PushStyleColor(
          ImGuiCol_ButtonHovered, ColorSet[Internal::ImGuiCol_Dialog_Directory_Hover]);
        ImGui::PushStyleColor(
          ImGuiCol_ButtonActive, ColorSet[Internal::ImGuiCol_Dialog_Directory_Pressed]);
        ...
        for (int i = 0, sz = (int)pUserKnownDirectories->size(); i < sz; i++) {
          ...
          // if (Dialog::DrawFolderIconCallback &&
          // Dialog::DrawFolderIconCallback(false,&iconsColor)) ImGui::SameLine();
          if (
            ImGui::SmallButton(userKnownFolderDisplayName) &&
            strcmp(userKnownFolder, I.currentFolder) != 0) {
...
#  undef SIMULATING_ONLY
          ...
          if (!Directory::Exists(newDirPath)) {
            fprintf(stderr, "Error creating new folder: \"%s\"\n", newDirPath);
          } else {
      ...
      //-----------------------------------------------------
      {
        ...
        if (I.allowDisplayByOption && I.numBrowsingColumns > 1) {
          ...
          if (ImGui::SmallButton(
                !Internal::BrowsingPerRow ? "Column##browsingPerRow" : "Row##browsingPerRow")) {
    ...
    //-----------------------------------------------------------------------------
    {
      if (ImGui::BeginChild(
            ...
            ImVec2(
              0,
              (isSaveFileDialog || isSelectFolderDialog)
                ? (ImGui::GetContentRegionAvail().y - 1.2f * ImGui::GetTextLineHeightWithSpacing() -
                   ImGui::GetStyle().WindowPadding.y)
                : 0))) {

        I.calculateBrowsingDataTableSizes(
          ImGui::GetWindowSize()); // (Actually we could save this call every frame, but it's
                                   // difficult to detect whn we need it)

        // ImGui::SetScrollPosHere();   // possible future ref: while drawing to place the scroll
        // bar making a certain entry visible Also we don't do any maual clipping here (for long
        // directories using a clipper might be useful)
        ...
        if (I.dirs.size() > 0) {
          ...
          for (int i = 0, sz = (int)I.dirs.size(); i < sz; i++) {
            ...
            if (I.filter.PassFilter(dirName)) {
              if (
                Dialog::DrawFolderIconCallback &&
                Dialog::DrawFolderIconCallback(false, &iconsColor)) {
                ImGui::SameLine();
              }
        ...
        if (!isSelectFolderDialog && I.files.size() > 0) {
          ...
          ImGui::PushStyleColor(
            ImGuiCol_Button, ColorSet[Internal::ImGuiCol_Dialog_File_Background]);
          ImGui::PushStyleColor(
            ImGuiCol_ButtonHovered, ColorSet[Internal::ImGuiCol_Dialog_File_Hover]);
          ImGui::PushStyleColor(
            ImGuiCol_ButtonActive, ColorSet[Internal::ImGuiCol_Dialog_File_Pressed]);
          ...
          if (old!=acceptZipFilesForBrowsing) {
                  ...
                  fprintf(stderr,"acceptZipFilesForBrowsing=%s %d
            ...
            if (I.filter.PassFilter(fileName)) {
              ...
              if (acceptZipFilesForBrowsing) {
                ...
                if (hasZipExtension && !isZipFile) {
                  ...
                  ImGui::PushStyleColor(
                    ImGuiCol_Text, ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Text]);
                  ImGui::PushStyleColor(
                    ImGuiCol_Button, ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Background]);
                  ImGui::PushStyleColor(
                    ImGuiCol_ButtonHovered, ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Hover]);
                  ImGui::PushStyleColor(
                    ...
                    ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Pressed]);
                  pIconsColor = &ColorSet[Internal::ImGuiCol_Dialog_ZipDirectory_Text];
                } else if (!hasZipExtension && isZipFile) {
                  ImGui::PopStyleColor(4);
                  ImGui::PushStyleColor(
                    ImGuiCol_Text, ColorSet[Internal::ImGuiCol_Dialog_File_Text]);
                  ImGui::PushStyleColor(
                    ImGuiCol_Button, ColorSet[Internal::ImGuiCol_Dialog_File_Background]);
                  ImGui::PushStyleColor(
                    ImGuiCol_ButtonHovered, ColorSet[Internal::ImGuiCol_Dialog_File_Hover]);
...
#endif // IMGUI_USE_MINIZIP
              if (
                Dialog::DrawFileIconCallback &&
                Dialog::DrawFileIconCallback(I.fileExtensionTypes[i], pIconsColor)) {
                ImGui::SameLine();
              }
...
#  endif                // IMGUI_USE_MINIZIP
    ...
    if (isSaveFileDialog || isSelectFolderDialog) {
      ...
      if (isSaveFileDialog) {
        ...
        if (
          ...
          fileFilterExtensionString && strlen(fileFilterExtensionString) > 0) {
          ImGui::SetTooltip("%s", fileFilterExtensionString);
        }
        ImGui::PopItemWidth();
        ImGui::SameLine();
      } else {
      ...
      if (selectionButtonPressed) {
        if (isSelectFolderDialog) {
          ...
          if (strlen(I.saveFileName) > 0) {
            bool pathOk = true;
            if (
              I.mustFilterSaveFilePathWithFileFilterExtensionString && fileFilterExtensionString &&
              strlen(fileFilterExtensionString) > 0) {
              pathOk = false;
              char saveFileNameExtension[MAX_FILENAME_BYTES];
              Path::GetExtension(I.saveFileName, saveFileNameExtension);
              const bool saveFileNameHasExtension = strlen(saveFileNameExtension) > 0;
              //-------------------------------------------------------------------
              FilenameStringVector wExts;
              String::Split(fileFilterExtensionString, wExts, ';');
              const size_t wExtsSize = wExts.size();
              if (!saveFileNameHasExtension) {
                if (wExtsSize == 0) {
                  pathOk = true; // Bad situation, better allow this case
                } else {
                  strcat(I.saveFileName, wExts[0]);
                }
              } else {
  ...
  const char* Dialog::chooseFileDialog(
    ...
    const float windowAlpha)
  {
    if (dialogTriggerButton) {
      internal->open = internal->rescan = internal->forceSetWindowPositionAndSize = true;
      internal->chosenPath[0] = '\0';
      internal->userHasJustCancelledDialog = false;
    }
    if (
      dialogTriggerButton ||
      (!internal->rescan && internal->open && strlen(getChosenPath()) == 0)) {
      const char* cp = ChooseFileMainMethod(
        *this,
        directory,
        false,
        false,
        ...
        windowPos,
        windowAlpha);
      if (!internal->open) { // AFAIK this should happen only when user clicks the close button (but
                             // not when he clicks cancel)
  ...
  const char* Dialog::chooseFolderDialog(
    bool dialogTriggerButton,
    const char* directory,
    const char* windowTitle,
    const ImVec2& windowSize,
    ...
    const float windowAlpha)
  {
    ...
    if (
      ...
      (!internal->rescan && internal->open && strlen(getChosenPath()) == 0)) {
      const char* cp = ChooseFileMainMethod(
        *this, directory, true, false, "", "", windowTitle, windowSize, windowPos, windowAlpha);
      if (!internal->open) {
        internal->userHasJustCancelledDialog = true;
#ifdef IMGUI_USE_MINIZIP
        internal->unz.close();
  ...
  const char* Dialog::saveFileDialog(
    bool dialogTriggerButton,
    const char* directory,
  ...
  {
    Path::GetDirectoryName(filePath, rv);
  }
  ...
  {
    Path::GetFileNameWithoutExtension(filePath, rv);
  }
  ...
  {
    Path::ChangeExtension(filePath, newExtension, rv);
  }
  ...
  {
    Path::Split(path, rv, leaveIntermediateTrailingSlashes);
  }
  void DirectoryGetDirectories(
    const char* directoryName,
    PathStringVector& result,
    FilenameStringVector* pOptionalNamesOut,
    Sorting sorting)
  {
    Directory::GetDirectories(directoryName, result, pOptionalNamesOut, sorting);
  }
  void DirectoryGetFiles(
    const char* directoryName,
    PathStringVector& result,
    FilenameStringVector* pOptionalNamesOut,
    Sorting sorting)
  {
    Directory::GetFiles(directoryName, result, pOptionalNamesOut, sorting);
  }
