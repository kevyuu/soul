 ...
 * the sceneShader program. They should not be used before creating
 * the program.
 */
static GLuint mesh;
...
/* Creates a sceneShader object of the specified type using the specified text
 */
static GLuint make_shader(GLenum type, const char* text)
{
    ...
    if (shader != 0)
    {
        ...
        {
            fprintf(stderr, "ERROR: Failed to compile %s sceneShader\n", (type == GL_FRAGMENT_SHADER) ? "fragment" : "vertex" );
...
static GLuint make_shader_program(const char* vs_text, const char* fs_text)
{
    ...
    if (vertex_shader != 0u)
    {
        ...
        {
            /* make the program that connect the two sceneShader and link it */
            program = glCreateProgram();
            if (program != 0u)
            {
                /* attach both sceneShader and link */
                ...
                {
                    fprintf(stderr, "ERROR, failed to link sceneShader program\n");
        ...
        {
            fprintf(stderr, "ERROR: Unable to load fragment sceneShader\n");
            glDeleteShader(vertex_shader);
        }
    }
    else
    {
        fprintf(stderr, "ERROR: Unable to load vertex sceneShader\n");
    }
    return program;
}
