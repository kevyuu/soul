...
template <typename T>
void verify_sequence(soul::IntrusiveList<T>& test_list, std::vector<T>& expected_values, std::vector<T*>& expected_objects)
{
    ...
    SOUL_TEST_ASSERT_TRUE(
        std::equal(expected_values.cbegin(), expected_values.cend(), test_list.begin(), test_list.end()));
    SOUL_TEST_ASSERT_TRUE(
        std::equal(expected_values.cbegin(), expected_values.cend(), test_list.cbegin(), test_list.cend()));
    SOUL_TEST_ASSERT_TRUE(
        std::equal(expected_values.crbegin(), expected_values.crend(), test_list.rbegin(), test_list.rend()));
    SOUL_TEST_ASSERT_TRUE(
        std::equal(expected_values.crbegin(), expected_values.crend(), test_list.crbegin(), test_list.crend()));
    SOUL_TEST_ASSERT_TRUE(std::all_of(expected_objects.cbegin(), expected_objects.cend(),
                                      [&test_list](const T* val) { return test_list.contains(*val); }));
...
TEST(TestIntrusiveListPushBack, TestIntrusiveListPushBack) {
    ...
    auto test_push_back = []<typename T>(soul::IntrusiveList<T>& test_list, std::vector<T>& new_values)
    {
...
{
    auto test_push_front = []<typename T>(soul::IntrusiveList<T>& test_list, std::vector<T>& new_values)
    {
        SOUL_TEST_ASSERT_NE(new_values.size(), 0);
...
template <typename T>
std::vector<T> get_vector_values(const std::vector<T*> objects)
{
    std::vector<T> result;
    result.reserve(objects.size());
    std::transform(objects.begin(), objects.end(), std::back_inserter(result), [](T* val) -> T& { return *val; });
    return result;
}
...
TEST(TestIntrusiveListPopFront, TestIntrusiveListPopFront)
{
    auto test_pop_back = []<typename T>(soul::IntrusiveList<T>& test_list)
    {
        ...
        std::ranges::for_each(expected_objects,
                              [&expected_values](T* objects) { expected_values.push_back(*objects); });
        test_list.pop_front();
        verify_sequence(test_list, expected_values, expected_objects);
...
{
    auto test_insert = []<typename T>(soul::IntrusiveList<T>& test_list, soul_size position, std::vector<T>& new_objects) {
        std::vector<T*> expected_objects;
        expected_objects.reserve(test_list.size() + new_objects.size());
        std::transform(test_list.begin(), std::next(test_list.begin(), position), std::back_inserter(expected_objects),
                       to_pointer<T>);
        std::transform(new_objects.begin(), new_objects.end(), std::back_inserter(expected_objects), to_pointer<T>);
        std::transform(std::next(test_list.begin(), position), test_list.end(), std::back_inserter(expected_objects),
                       to_pointer<T>);
...
TEST(TestIntrusiveListRemove, TestIntrusiveListRemove)
{
    auto test_remove = []<typename T>(soul::IntrusiveList<T>& list, soul_size position)
    {
        ...
        std::transform(list.begin(), std::next(list.begin(), position), std::back_inserter(expected_objects),
                       to_pointer<T>);
        std::transform(std::next(list.begin(), position + 1), list.end(), std::back_inserter(expected_objects),
                       to_pointer<T>);
        std::vector<T> expected_values = get_vector_values(expected_objects);
...
TEST(TestIntrusiveListErase, TestIntrusiveListEraseSingle)
{
    auto test_erase_single = []<typename T>(soul::IntrusiveList<T>& test_list, soul_size position)
    {
        ...
        std::transform(test_list.begin(), std::next(test_list.begin(), position), std::back_inserter(expected_objects),
                       to_pointer<T>);
        std::transform(std::next(test_list.begin(), position + 1), test_list.end(), std::back_inserter(expected_objects),
                       to_pointer<T>);
...
{
    auto test_erase_range = []<typename T>(soul::IntrusiveList<T>& test_list, soul_size first, soul_size last) {
        std::vector<T*> expected_objects;
        expected_objects.reserve(test_list.size() - (last - first));
        std::transform(test_list.begin(), std::next(test_list.begin(), first), std::back_inserter(expected_objects),
                       to_pointer<T>);
        std::transform(std::next(test_list.begin(), last), test_list.end(), std::back_inserter(expected_objects),
                       to_pointer<T>);
        std::vector<T> expected_values = get_vector_values(expected_objects);
...
{
    auto test_splice_value = []<typename T>(soul::IntrusiveList<T>& src_list, soul::IntrusiveList<T>& dst_list,
                                             soul_size src_position, soul_size dst_position)
    {
        ...
        std::transform(src_list.begin(), std::next(src_list.begin(), src_position), std::back_inserter(src_expected_objects), to_pointer<T>);
        std::transform(std::next(src_list.begin(), src_position + 1), src_list.end(), std::back_inserter(src_expected_objects), to_pointer<T>);
        std::vector<T> src_expected_values = get_vector_values(src_expected_objects);

        std::vector<T*> dst_expected_objects;
        std::transform(dst_list.begin(), std::next(dst_list.begin(), dst_position),
                       std::back_inserter(dst_expected_objects), to_pointer<T>);
        dst_expected_objects.push_back(&(*std::next(src_list.begin(), src_position)));
        std::transform(std::next(dst_list.begin(), dst_position), dst_list.end(),
                       std::back_inserter(dst_expected_objects), to_pointer<T>);
        std::vector<T> dst_expected_values = get_vector_values(dst_expected_objects);

        dst_list.splice(std::next(dst_list.begin(), dst_position), *std::next(src_list.begin(), src_position));
    ...
    SOUL_TEST_RUN(
        test_splice_value(random_list1.list, random_list2.list, random_list1.list.size() / 2, random_list2.list.size() / 2));
    SOUL_TEST_RUN(test_splice_value(random_list1.list, random_list2.list, 0, random_list1.list.size() / 2));
    SOUL_TEST_RUN(test_splice_value(random_list1.list, random_list2.list, random_list1.list.size() - 1,
                                     random_list1.list.size() / 2));

    SOUL_TEST_RUN(test_splice_value(random_list1.list, random_list2.list, 0, 0));
    SOUL_TEST_RUN(test_splice_value(random_list1.list, random_list2.list, 0, random_list2.list.size()));
...
{
    auto test_splice_list = []<typename T>(soul::IntrusiveList<T>& src_list, soul::IntrusiveList<T>& dst_list, soul_size position) { std::vector<T*> src_expected_objects;

        std::vector<T> src_expected_values;

        std::vector<T*> dst_expected_objects;
        std::transform(dst_list.begin(), std::next(dst_list.begin(), position),
                       std::back_inserter(dst_expected_objects), to_pointer<T>);
        std::transform(src_list.begin(), src_list.end(), std::back_inserter(dst_expected_objects), to_pointer<T>);
        std::transform(std::next(dst_list.begin(), position), dst_list.end(), std::back_inserter(dst_expected_objects), to_pointer<T>);
        std::vector<T> dst_expected_values = get_vector_values(dst_expected_objects);
    ...
    SOUL_TEST_RUN(test_splice_list(random_list2.list, random_list1.list, random_list1.list.size() / 2));
...
{
    auto test_splice_list_single = []<typename T>(soul::IntrusiveList<T>& src_list, soul::IntrusiveList<T>& dst_list,
                                             soul_size src_position, soul_size dst_position)
    {
        SOUL_TEST_ASSERT_NE(std::next(src_list.begin(), src_position), src_list.end());
        std::vector<T*> src_expected_objects;
        src_expected_objects.reserve(src_list.size() - 1);
        std::transform(src_list.begin(), std::next(src_list.begin(), src_position),
                       std::back_inserter(src_expected_objects), to_pointer<T>);
        std::transform(std::next(src_list.begin(), src_position + 1), src_list.end(),
                       std::back_inserter(src_expected_objects), to_pointer<T>);
        std::vector<T> src_expected_values = get_vector_values(src_expected_objects);
        ...
        dst_list.splice(std::next(dst_list.begin(), dst_position), src_list, std::next(src_list.begin(), src_position));

        SOUL_TEST_RUN(verify_sequence(src_list, src_expected_values, src_expected_objects));
        SOUL_TEST_RUN(verify_sequence(dst_list, dst_expected_values, dst_expected_objects));
    };

    SOUL_TEST_RUN(test_splice_list_single(random_list1.list, random_list2.list, random_list1.list.size() / 2,
                                     random_list2.list.size() / 2));
    SOUL_TEST_RUN(test_splice_list_single(random_list1.list, random_list2.list, 0, random_list1.list.size() / 2));
    SOUL_TEST_RUN(test_splice_list_single(random_list1.list, random_list2.list, random_list1.list.size() - 1,
                                     random_list1.list.size() / 2));

    SOUL_TEST_RUN(test_splice_list_single(random_list1.list, random_list2.list, 0, 0));
    SOUL_TEST_RUN(test_splice_list_single(random_list1.list, random_list2.list, 0, random_list2.list.size()));
...
{
    auto test_splice_list_range = 
        []<typename T>(soul::IntrusiveList<T>& src_list, soul::IntrusiveList<T>& dst_list,
                     soul_size src_position_start, soul_size src_position_end,
                     soul_size dst_position)
    {
        std::vector<T*> src_expected_objects;
        src_expected_objects.reserve(src_list.size() - (src_position_end - src_position_start));
        std::transform(src_list.begin(), std::next(src_list.begin(), src_position_start),
                       std::back_inserter(src_expected_objects), to_pointer<T>);
        std::transform(std::next(src_list.begin(), src_position_end), src_list.end(),
                       std::back_inserter(src_expected_objects), to_pointer<T>);
        std::vector<T> src_expected_values = get_vector_values(src_expected_objects);
        ...
        std::transform(dst_list.begin(), std::next(dst_list.begin(), dst_position),
                       std::back_inserter(dst_expected_objects), to_pointer<T>);
        std::transform(std::next(src_list.begin(), src_position_start), std::next(src_list.begin(), src_position_end),
                       std::back_inserter(dst_expected_objects), to_pointer<T>);
        std::transform(std::next(dst_list.begin(), dst_position), dst_list.end(),
                       std::back_inserter(dst_expected_objects), to_pointer<T>);
        std::vector<T> dst_expected_values = get_vector_values(dst_expected_objects);

        dst_list.splice(std::next(dst_list.begin(), dst_position), src_list,
                        std::next(src_list.begin(), src_position_start), std::next(src_list.begin(), src_position_end));

         SOUL_TEST_RUN(verify_sequence(src_list, src_expected_values, src_expected_objects));
         SOUL_TEST_RUN(verify_sequence(dst_list, dst_expected_values, dst_expected_objects));
    };

    SOUL_TEST_RUN(test_splice_list_range(random_list2.list, random_list1.list, 0, random_list2.list.size(), random_list1.list.size() / 2));
    SOUL_TEST_RUN(test_splice_list_range(random_list3.list, random_list1.list, 0, 1, 0));
    SOUL_TEST_RUN(test_splice_list_range(random_list4.list, random_list1.list, 0, random_list4.list.size() / 2,
    ...
    SOUL_TEST_RUN(test_splice_list_range(random_list1.list, empty_list.list, 0, random_list1.list.size(), 0));
