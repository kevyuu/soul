...
TEST(TestCoreUtil, TestGetFirstOneBitPos)
{
  ...
  SOUL_TEST_ASSERT_EQ(
    soul::util::get_first_one_bit_pos(static_cast<uint16>(~uint16{0})).value(), 0);
  ...
  SOUL_TEST_ASSERT_EQ(
    soul::util::get_first_one_bit_pos(uint64{18446744069414584320llu}).value(), 32);
}
...
TEST(TestCoreUtil, TestGetLastOneBitPos)
{
  ...
  SOUL_TEST_ASSERT_EQ(
    soul::util::get_last_one_bit_pos(static_cast<uint16>(~uint16{0})).value(), 15);
...
TEST(TestCoreUtil, TestGetOneBitCount)
{
  ...
  {
    constexpr auto test_count = soul::util::get_one_bit_count(uint16{0b0100'0100'0100'0001});
...
TEST(TestCoreUtil, TestForEachOneBitPos)
{
  auto test_for_each_one_bit_pos =
    []<std::integral T>(T val, const std::vector<uint32>& expected_value) {
      std::vector<uint32> bit_pos;
      soul::util::for_each_one_bit_pos(
        val, [&bit_pos](const soul_size position) { bit_pos.push_back(position); });
  ...
  SOUL_TEST_RUN(
    test_for_each_one_bit_pos(static_cast<uint16>(~uint16{0}), expected_result_all_set));

  SOUL_TEST_RUN(
    test_for_each_one_bit_pos(uint64{0x4F000200}, std::vector<uint32>({9, 24, 25, 26, 27, 30})));
}
