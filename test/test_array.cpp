...
template <typename T, soul_size element_count, std::ranges::sized_range RangeT>
...
{
  auto test_default_constructor = []<typename T, soul_size element_count>() {
...
{
  auto test_init_fill_construction = []<typename T, soul_size element_count>(const T& val) {
...
TEST(TestArrayConstruction, TestInitGenerateConstruction)
{
  auto test_init_generate_construction =
    []<typename T, soul_size element_count>(soul::ts_generate_fn<T> auto fn) {
      const auto arr = soul::Array<T, element_count>::init_generate(fn);
      const auto range =
        std::views::iota(soul_size(0), element_count) |
        std::views::transform([fn](soul_size /* idx */) -> T { return std::invoke(fn); });
      verify_array<T, element_count>(arr, range);
    };

  auto test_obj_gen_fn = [] { return TestObject(5); };
  SOUL_TEST_RUN(
...
TEST(TestArrayConstruction, TestInitIndexTransformConstruction)
{
  ...
  auto test_obj_gen_fn = [](soul_size index) { return TestObject(soul::cast<int>(index)); };
  SOUL_TEST_RUN(SOUL_SINGLE_ARG(
  ...
  auto list_test_obj_gen_fn = [](soul_size index) { return ListTestObject::with_size(index); };
  SOUL_TEST_RUN(SOUL_SINGLE_ARG(
...
{
  auto test_copy_constructor = []<typename T, soul_size element_count>() {
