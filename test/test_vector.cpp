...
template <typename T>
static void generate_random_array(T* arr, soul_size size)
{
...
public:
    ...
    soul::Vector <TestObject> vectorToSrc{CONSTRUCTOR_VECTOR_SIZE, TestObject(CONSTRUCTOR_VECTOR_DEFAULT_VALUE)};
    soul::Vector <ListTestObject> vectorListToSrc{CONSTRUCTOR_VECTOR_SIZE, ListTestObject(CONSTRUCTOR_VECTOR_DEFAULT_VALUE)};
};
...
public:
    ...
    TestObject testObjectArr[5] = { TestObject(1), TestObject(2), TestObject(3), TestObject(4), TestObject(5) };
    ListTestObject listTestObjectArr[5] = { ListTestObject(1), ListTestObject(2), ListTestObject(3), ListTestObject(4), ListTestObject(5)};
...
{
    auto test_assignment_operator = []<typename T>(soul::Vector<T> test_vector, const soul_size size)
    {
...
{
    auto test_move_assignment_operator = []<typename T>(soul::Vector<T> test_vector, const soul_size size)
    {
        soul::memory::Allocator* allocator = test_vector.get_allocator();
        auto src_arr = new T[size];
        SCOPE_EXIT(delete[] src_arr);
        ...
        SOUL_TEST_ASSERT_TRUE(std::equal(test_vector.begin(), test_vector.end(), src_arr, src_arr + size));
        SOUL_TEST_ASSERT_EQ(test_vector.get_allocator(), allocator);
    };
...
TEST_F(TestVectorManipulation, TestVectorAssignWithSizeAndValue)
{
    ...
    static constexpr int ASSIGN_VECTOR_DEFAULT_VALUE = 8;

    auto test_assign_with_size_and_value = []<typename T>(soul::Vector<T> test_vector, const soul_size size, const T & val)
    {
    ...
    SOUL_TEST_RUN(test_assign_with_size_and_value(vectorIntEmpty, ASSIGN_VECTOR_SIZE, ASSIGN_VECTOR_DEFAULT_VALUE));
    SOUL_TEST_RUN(test_assign_with_size_and_value(vectorTOEmpty, ASSIGN_VECTOR_SIZE, TestObject(ASSIGN_VECTOR_DEFAULT_VALUE)));
    SOUL_TEST_RUN(test_assign_with_size_and_value(vectorListTOArr, ASSIGN_VECTOR_SIZE, ListTestObject(ASSIGN_VECTOR_DEFAULT_VALUE)));

    static constexpr soul_size ASSIGN_VECTOR_OFFSET_SIZE = 2;
    SOUL_TEST_RUN(test_assign_with_size_and_value(vectorIntArr, vectorIntArr.size() + ASSIGN_VECTOR_OFFSET_SIZE, ASSIGN_VECTOR_DEFAULT_VALUE));
    SOUL_TEST_RUN(test_assign_with_size_and_value(vectorIntArr, vectorIntArr.size() - ASSIGN_VECTOR_OFFSET_SIZE, ASSIGN_VECTOR_DEFAULT_VALUE));

    SOUL_TEST_RUN(test_assign_with_size_and_value(vectorTOArr, vectorTOArr.size() + ASSIGN_VECTOR_OFFSET_SIZE, TestObject(ASSIGN_VECTOR_DEFAULT_VALUE)));
    SOUL_TEST_RUN(test_assign_with_size_and_value(vectorTOArr, vectorTOArr.size() - ASSIGN_VECTOR_OFFSET_SIZE, TestObject(ASSIGN_VECTOR_DEFAULT_VALUE)));

    SOUL_TEST_RUN(test_assign_with_size_and_value(vectorListTOArr, vectorListTOArr.size() + ASSIGN_VECTOR_OFFSET_SIZE, ListTestObject(ASSIGN_VECTOR_DEFAULT_VALUE)));
    SOUL_TEST_RUN(test_assign_with_size_and_value(vectorListTOArr, vectorListTOArr.size() - ASSIGN_VECTOR_OFFSET_SIZE, ListTestObject(ASSIGN_VECTOR_DEFAULT_VALUE)));
...
TEST_F(TestVectorManipulation, TestVectorSwap)
{
    auto test_swap = []<typename T>(soul::Vector<T>& test_vector, soul_size size) {
        ...
        SOUL_TEST_ASSERT_TRUE(std::equal(swapped_vector.begin(), swapped_vector.end(), arr, arr + size));
...
TEST_F(TestVectorManipulation, TestVectorResize)
{
    auto test_resize = []<typename T>(soul::Vector<T> test_vector, const soul_size size) {
        ...
        SOUL_TEST_ASSERT_TRUE(std::equal(test_vector.begin(), test_vector.begin() + smaller_size, test_copy.begin()));
        if (size > test_copy.size())
        {
            SOUL_TEST_ASSERT_TRUE(std::all_of(test_vector.begin() + smaller_size, test_vector.end(), [](const T& x) { return x == T();  }));
        }
        else
        {
            if constexpr (std::same_as<T, TestObject>)
            {
                SOUL_TEST_ASSERT_EQ(TestObject::sTODtorCount - TestObject::sTOCtorCount, test_copy.size() - size);
            }
...
TEST_F(TestVectorManipulation, TestVectorPopBack)
{
    ...
    auto test_pop_back_with_size = []<typename T>(soul::Vector<T> test_vector, const soul_size pop_back_size) {
...
TEST_F(TestVectorManipulation, TestVectorEmplaceBack)
{
    auto test_emplace_back = [](const VectorObj test_vector)
    {
        ...
        SOUL_TEST_ASSERT_TRUE(std::equal(test_vector.cbegin(), test_vector.cend(), test_copy1.cbegin()));

        VectorObj test_copy2(test_vector);
        test_copy2.emplace_back(4, 5, 6);
        SOUL_TEST_ASSERT_EQ(test_copy2.size(), test_vector.size() + 1);
        SOUL_TEST_ASSERT_EQ(test_copy2.back(), TestObject(4 + 5 + 6));
        SOUL_TEST_ASSERT_TRUE(std::equal(test_vector.cbegin(), test_vector.cend(), test_copy2.cbegin()));
    };
...
TEST_F(TestVectorManipulation, TestVectorAppend)
{
    auto test_append = []<typename T>(soul::Vector<T> test_vector, const soul_size append_size)
    {
        ...
        SOUL_TEST_ASSERT_TRUE(std::equal(test_vector.begin(), test_vector.end(), test_copy1.begin()));
        SOUL_TEST_ASSERT_TRUE(std::equal(test_copy1.begin() + test_vector.size(), test_copy1.end(), src_append_arr, src_append_arr + append_size));
