...
namespace khaos
{
  namespace
  {
    void dialog_line(NotNull<app::Gui*> gui, StringView text)
    {
      ...
      for (usize char_i = 0; char_i < text.size();)
      {
        ...
        if (current_char == '"')
        {
          auto result = find_char(text, '"', char_i + 1);
        ...
        } else
        {
          ...
          auto result = find_char(text, special_characters.cspan(), char_i + 1);
  ...
  void dialog_text(NotNull<app::Gui*> gui, StringView text)
  {
    ...
    for_each_line(
      text,
      [gui, &choice_index](usize line_i, StringView line)
      {
        const auto choice_prefix = "<choice>"_str;
        const auto choice_suffix = "</choice>"_str;
        if (starts_with(line, choice_prefix))
        {
          choice_line(
            gui,
            choice_index,
