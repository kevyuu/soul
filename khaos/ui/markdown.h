...
namespace ImGui
{
  ...
  inline void defaultMarkdownFormatCallback(
    const MarkdownFormatInfo& markdownFormatInfo_, bool start_);
  ...
  inline void Markdown(
    const char* markdown_, size_t markdownLength_, const MarkdownConfig& mdConfig_);
  ...
  inline void RenderLine(
    const char* markdown_, Line& line_, TextRegion& textRegion_, const MarkdownConfig& mdConfig_);
  ...
  struct TextRegion
  {
    ...
    void RenderTextWrapped(const char* text_, const char* text_end_, bool bIndentToHere_ = false)
    {
      ...
      const char* endLine =
        ImGui::GetFont()->CalcWordWrapPositionA(scale, text_, text_end_, widthLeft);
  ...
  inline void Markdown(
    const char* markdown_, size_t markdownLength_, const MarkdownConfig& mdConfig_)
  {
    static const char* linkHoverStart =
    ...
    for (int i = 0; i < (int)markdownLength_; ++i)
    {
      ...
      if (c == '\n')
      {
        ...
        line.lineEnd = i;
        if (
          em.state == Emphasis::MIDDLE && line.emphasisCount >= 3 &&
    ...
    } else
    {
      ...
      if (
        markdownLength_ && line.lineStart < (int)markdownLength_ && markdown_[line.lineStart] != 0)
      {
  ...
  inline bool TextRegion::RenderLinkText(
    const char* text_,
    const char* text_end_,
    const Link& link_,
    const char* markdown_,
    ...
    bool bIndentToHere_)
  {
    ...
    if (endLine > text_ && endLine < text_end_)
    {
      if (IsCharInsideWord(*endLine))
      {
        ...
        if (
          endNextLine == text_end_ || (endNextLine <= text_end_ && !IsCharInsideWord(*endNextLine)))
        {
