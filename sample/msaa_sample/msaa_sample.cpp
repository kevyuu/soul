    ...
    -> gpu::TextureNodeID override
  {
    if (ImGui::Begin("Options"))
    {
      if (ImGui::BeginCombo("Sample Count", TEXTURE_SAMPLE_COUNT_NAME[msaa_sample_count_]))
      {
        ...
        supported_sample_count_flags.for_each(
          [this](gpu::TextureSampleCount count)
          {
  ...
  explicit MSAASample(const AppConfig& app_config) : App(app_config), texture_2d_pass_(gpu_system_)
  {
    ...
    push_constants_.push_back(MSAAPushConstant{
      .transform = math::scale(
        math::rotate(
          math::translate(mat4f32::Identity(), vec3f32(-0.25f, 0.0f, 0.1f)),
          rotate_angle,
          rotate_axis),
        scale_vec),
    ...
    push_constants_.push_back(MSAAPushConstant{
      .transform = math::scale(
        math::rotate(
          math::translate(mat4f32::Identity(), vec3f32(0.25f, 0.0f, 0.0f)),
          rotate_angle,
          rotate_axis),
        scale_vec),
