...
class MSAASample final : public App
{
	...
	soul::Vector<MSAAPushConstant> push_constants_;
	
	gpu::TextureSampleCount msaa_sample_count_ = gpu::TextureSampleCount::COUNT_4;

	static constexpr auto TEXTURE_SAMPLE_COUNT_NAME = FlagMap<gpu::TextureSampleCount, const char*>::build_from_list({
		"1",
		"2",
		"4",
		"8",
	...
	void render(gpu::TextureNodeID render_target, gpu::RenderGraph& render_graph) override
	{
		...
		if (ImGui::Begin("Options"))
		{
			...
			{
				const auto supported_sample_count_flags = gpu_properties_.limit.color_sample_count_flags & gpu_properties_.limit.depth_sample_count_flags;
				supported_sample_count_flags.for_each([this](gpu::TextureSampleCount count)
				{
				    const bool is_selected = (msaa_sample_count_ == count);
		...
		const auto texture_2d_sample_input = enable_msaa ? 
			sample_render_target.resolve_attachments[0].out_node_id : sample_render_target.color_attachments[0].out_node_id;
...
public:
	explicit MSAASample(const AppConfig& app_config) : App(app_config), texture_2d_pass_(gpu_system_)
	{
		...
		constexpr auto entry_points = std::to_array<gpu::ShaderEntryPoint>({
			{gpu::ShaderStage::VERTEX, "vs_main"},
			{gpu::ShaderStage::FRAGMENT, "ps_main"}
			});
		...
		push_constants_.push_back({
			.transform = math::scale(math::rotate(math::translate(mat4f::identity(), vec3f(-0.25f, 0.0f, 0.1f)), rotate_angle, rotate_axis), scale_vec),
			.color = vec3f(1.0f, 0.0f, 0.0f)
		});
		push_constants_.push_back({
			.transform = math::scale(math::rotate(math::translate(mat4f::identity(), vec3f(0.25f, 0.0f, 0.0f)), rotate_angle, rotate_axis), scale_vec),
