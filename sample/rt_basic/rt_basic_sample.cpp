...
struct ObjInstance
{
	soul::mat4f transform;    // Matrix of the instance
	uint32 obj_index{ 0 };  // Model index reference
};
...
class RTBasicSampleApp final : public App
{
	...
	soul::SBOVector<ObjModel> models_;
	soul::SBOVector<GPUObjDesc> gpu_obj_descs_;
	soul::gpu::BufferID gpu_obj_buffer_;
	soul::SBOVector<Texture> textures_;
	soul::SBOVector<ObjInstance> instances_;
	soul::SBOVector<gpu::BlasID> blas_ids_;
	gpu::TlasID tlas_id_;
	...
	soul::vec4f clear_color_ = vec4f(1.0f, 1.0f, 1.0f, 1.0f);
	struct Light
	{
		soul::vec3f position = { 10.f, 15.f, 8.f };
	...
	gpu::TextureNodeID render(gpu::TextureNodeID render_target, gpu::RenderGraph& render_graph) override
	{
		...
		if (need_rebuild_blas_)
		{
			...
			blas_group_node_id = render_graph.add_non_shader_pass<BuildBlasParameter>("Build blas group", gpu::QueueType::COMPUTE,
				...
				[this](const auto& parameter, const auto& registry, auto& command_list)
				{
					...
					for (soul_size model_idx = 0; model_idx < models_.size(); model_idx++)
					{
						const auto& model = models_[model_idx];
						const auto geom_desc = scope_allocator.create<gpu::RTGeometryDesc>(gpu::RTGeometryDesc{
					...
					command_list.push(gpu::RenderCommandBatchBuildBlas{
						.build_count = soul::cast<uint32>(render_commands.size()),
						.builds = render_commands.data(),
						.max_build_memory_size = MAX_BLAS_BUILD_MEMORY
					});

				}).get_parameter().blas_group_node_id;
		...
		{
			auto instance_buffer = render_graph.create_buffer("Instance buffer", {
			...
			instance_buffer = render_graph.add_non_shader_pass<UploadInstanceBufferParameter>("Upload instance buffer", gpu::QueueType::TRANSFER,
			...
			[this](const auto& parameter, const auto& registry, auto& command_list)
			{
				soul::Vector<soul::gpu::RTInstanceDesc> instance_descs;
			...
			tlas_node_id = render_graph.add_non_shader_pass<BuildTlasParameter>("Build Tlas Pass", gpu::QueueType::COMPUTE,
			...
			[this](const auto& parameter, const auto& registry, auto& command_list)
			{
				command_list.push(gpu::RenderCommandBuildTlas{
					...
					.build_desc = {
						.instance_data = gpu_system_->get_gpu_address(registry.get_buffer(parameter.instance_buffer)),
						.instance_count = soul::cast<uint32>(instances_.size())
					}
				});
			}).get_parameter().tlas_node_id;
			need_rebuild_tlas_ = false;
		}

		const vec2ui32 viewport = gpu_system_->get_swapchain_extent();

		const auto scene_buffer = render_graph.create_buffer("Scene Buffer", { .size = sizeof(GPUObjScene) });
		...
		const auto scene_upload_parameter = render_graph.add_non_shader_pass<GPUSceneUploadPassParameter>(
			"GPUScene upload",
		...
		const auto rt_pass_param = render_graph.add_ray_tracing_pass<RayTracingPassParameter>("Ray Tracing Pass",
		    [target_texture, scene_upload_parameter, tlas_node_id, blas_group_node_id](auto& parameter, auto& builder)
		    {
				parameter.target_texture = builder.add_shader_texture(target_texture, 
					{ gpu::ShaderStage::RAYGEN }, gpu::ShaderTextureWriteUsage::STORAGE);
				parameter.scene_buffer = builder.add_shader_buffer(scene_upload_parameter.buffer,
				...
				parameter.blas_group = builder.add_shader_blas_group(blas_group_node_id, gpu::SHADER_STAGES_RAY_TRACING);
		    },
		    [this, viewport](const auto& parameter, auto& registry, auto& command_list)
		    {
				RayTracingPushConstant push_constant = {
					...
					.as_descriptor_id = gpu_system_->get_as_descriptor_id(registry.get_tlas(parameter.tlas)),
					.image_descriptor_id = gpu_system_->get_uav_descriptor_id(registry.get_texture(parameter.target_texture)),
	...
	void load_model(const std::filesystem::path& model_path, const mat4f transform = mat4f::identity())
	{
		instances_.push_back({
			.transform = transform,
			.obj_index = soul::cast<uint32>(models_.size())
		});
		...
		const gpu::BufferDesc vertex_buffer_desc = {
			...
			.usage_flags = { gpu::BufferUsage::VERTEX, gpu::BufferUsage::STORAGE, gpu::BufferUsage::AS_BUILD_INPUT },
			.queue_flags = { gpu::QueueType::GRAPHIC },
			.name = "Vertex buffer"
		};
		const auto vertex_buffer = gpu_system_->create_buffer(vertex_buffer_desc, obj_loader.vertices.data());

		const gpu::BufferDesc index_buffer_desc = {
			.size = obj_loader.indices.size() * sizeof(IndexObj),
			.usage_flags = { gpu::BufferUsage::INDEX, gpu::BufferUsage::STORAGE, gpu::BufferUsage::AS_BUILD_INPUT },
			.queue_flags = { gpu::QueueType::GRAPHIC },
			.name = "Index buffer"
		};
		const auto index_buffer = gpu_system_->create_buffer(index_buffer_desc, obj_loader.indices.data());

		for (const auto& texture_name : obj_loader.textures)
		{
			textures_.push_back(Texture{});
			auto& texture = textures_.back();
			texture.name = CString(texture_name.c_str());
			const auto texture_path = get_media_path() / "textures" / texture_name;
			int texture_width, texture_height, texture_channel_count;
			stbi_uc* texture_pixels = stbi_load(texture_path.string().c_str(), &texture_width, &texture_height, &texture_channel_count, STBI_rgb_alpha);
			...
			const gpu::TextureDesc texture_desc = gpu::TextureDesc::d2(
				...
				{ soul::cast<uint32>(texture_width), soul::cast<uint32>(texture_height) });
		...
		for (const auto& material : obj_loader.materials)
		{
			gpu_materials.push_back({
				...
				.diffuse_texture_id = material.texture_id == -1 ? gpu::DescriptorID::null() : gpu_system_->get_srv_descriptor_id(textures_[material.texture_id].texture_id)
			});
		}
		...
		const auto material_buffer = gpu_system_->create_buffer(material_buffer_desc, gpu_materials.data());
		...
		const auto material_indices_buffer = gpu_system_->create_buffer(material_indices_buffer_desc, obj_loader.mat_indexes.data());
		...
		const GPUObjDesc gpu_obj_desc = {
			...
			.material_indices_descriptor_id = gpu_system_->get_ssbo_descriptor_id(material_indices_buffer)
		};
	...
	void create_gpu_obj_desc_buffer()
	{
	...
	void create_tlas()
	{
		...
		tlas_id_ = gpu_system_->create_tlas({
			.name = "Tlas",
			.size = tlas_size
		});
...
public:
	explicit RTBasicSampleApp(const AppConfig& app_config) : App(app_config), texture_2d_pass(gpu_system_)
	{
		...
		const auto miss_groups = std::to_array<gpu::RTGeneralShaderGroup>({
			{.entry_point = 1},
			{.entry_point = 2}
		});
		...
		const auto  camera_position = camera_target + (bounding_box_.max - camera_target) * distance_multiplier;
