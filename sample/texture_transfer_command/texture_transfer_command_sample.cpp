...
class Texture3DSampleApp final : public App
{
	...
	void render(gpu::RenderGraph& render_graph) override
	{
		...
		gpu::TextureNodeID persistent_texture_node_id = render_graph.import_texture("Persistent Texture", test_texture_id_);
		...
		gpu::TextureNodeID dst_texture_node_id = render_graph.create_texture("Copy dst texture", gpu::RGTextureDesc::create_d2(gpu::TextureFormat::RGBA8, 1, 
			{width_, height_}));
