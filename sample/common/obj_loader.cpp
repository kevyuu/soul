...
void ObjLoader::load_model(const std::filesystem::path& filepath)
{
    ...
    for (const auto& material : reader.GetMaterials())
    {
        ...
        m.transmittance = soul::vec3f(material.transmittance[0], material.transmittance[1], material.transmittance[2]);
        ...
        {
            if (const auto it = texture_name_index_map.find(material.diffuse_texname); it != texture_name_index_map.end())
            {
    ...
    if (attrib.normals.empty())
    {
        for (size_t i = 0; i < indices.size(); i += 3)
        {
            ...
            soul::vec3f n = soul::math::normalize(soul::math::cross((v1.position - v0.position), (v2.position - v0.position)));
            v0.normal = n;
            v1.normal = n;
            v2.normal = n;
        }
