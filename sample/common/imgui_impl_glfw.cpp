...
//  ImGui::IsKeyPressed(GLFW_KEY_SPACE). [X] Platform: Multi-viewport support (multiple windows).
...
//  2021-08-17: *BREAKING CHANGE*: Now using glfwSetWindowFocusCallback() to calling
//  io.AddFocusEvent(). If you called ImGui_ImplGlfw_InitXXX() with install_callbacks = false, you
//  MUST install glfwSetWindowFocusCallback() and forward it to the backend via
...
#define GLFW_HAS_FOCUS_WINDOW                                                                      \
  (GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >= 3200) // 3.2+ glfwFocusWindow
...
static void ImGui_ImplGlfw_UpdateMousePosAndButtons()
{
  ...
  for (int n = 0; n < platform_io.Viewports.Size; n++) {
    ...
    // all Dear ImGui positions are same as OS positions)
    ...
    // If ImGuiBackendFlags_HasMouseHoveredViewport is not set by the backend, imgui will ignore
    // this field and infer the information by relying on the rectangles and last focused time of
    // every viewports it knows about. It will be unaware of other windows that may be sitting
    // between or over your windows. [GLFW] FIXME: This is currently only correct on Win32. See what
...
static void ImGui_ImplGlfw_CreateWindow(ImGuiViewport* viewport)
{
  ...
  vd->Window = glfwCreateWindow(
    (int)viewport->Size.x, (int)viewport->Size.y, "No Title Yet", NULL, share_window);
  vd->WindowOwned = true;
      ...
      for (int i = 0; i < IM_ARRAYSIZE(bd->KeyOwnerWindows); i++) {
        if (bd->KeyOwnerWindows[i] == vd->Window) {
          ImGui_ImplGlfw_KeyCallback(
            vd->Window, i, 0, GLFW_RELEASE, 0); // Later params are only used for main viewport, on
...
{
  if (msg == WM_NCHITTEST) {
    // Let mouse pass-through the window. This will allow the backend to set io.MouseHoveredViewport
    // properly (which is OPTIONAL). The ImGuiViewportFlags_NoInputs flag is set while dragging a
    // viewport, as want to detect the window behind the one we are dragging. If you cannot easily
    // access those viewport flags from your windowing/event code: you may manually synchronize its
    // state e.g. in your main loop after calling UpdatePlatformWindows(). Iterate all
...
static void ImGui_ImplGlfw_SetWindowSize(ImGuiViewport* viewport, ImVec2 size)
{
  ...
  // platforms they are positioned from the upper-left corner. GLFW makes an effort to convert macOS
  // style coordinates, however it doesn't handle it when changing size. We are manually moving the
