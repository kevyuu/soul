...
ImGuiRenderGraphPass::ImGuiRenderGraphPass(GLFWwindow* window, soul::gpu::System* gpu_system) : gpu_system_(gpu_system)
{
	...
	const gpu::TextureRegionUpdate region = {
		...
		.extent = {
			soul::cast<uint32>(width), soul::cast<uint32>(height), 1
		}
	};
	...
	const auto font_tex_desc = gpu::TextureDesc::d2("Font Texture", gpu::TextureFormat::RGBA8, 1,
		{ gpu::TextureUsage::SAMPLED }, { gpu::QueueType::GRAPHIC }, vec2ui32(width, height));
	...
	font_sampler_id_ = gpu_system->request_sampler(gpu::SamplerDesc::same_filter_wrap(gpu::TextureFilter::LINEAR, gpu::TextureWrap::CLAMP_TO_EDGE));
...
void ImGuiRenderGraphPass::add_pass(soul::gpu::RenderGraph& render_graph)
{
	...
	const gpu::BufferNodeID vertex_buffer_node_id = render_graph.create_buffer("ImGui Vertex", {
		.size = sizeof(ImDrawVert) * draw_data.TotalVtxCount
	});
	const gpu::BufferNodeID index_buffer_node_id = render_graph.create_buffer("ImGui Index", {
		.size = sizeof(ImDrawIdx) * draw_data.TotalIdxCount
	});
	...
	const gpu::BufferNodeID transform_buffer_node_id = render_graph.create_buffer("ImGui Transform Buffer", {
	...
	render_graph.add_graphic_pass<RenderPassParameter>("ImGui Render Pass",
		...
		}, [viewport, &draw_data, this](const RenderPassParameter& parameter, gpu::RenderGraphRegistry& registry, gpu::GraphicCommandList& command_list)
		{
			...
			gpu::GraphicPipelineStateDesc pipeline_desc = {
				...
				.input_attributes = {
					{.binding = 0, .offset = offsetof(ImDrawVert, pos), .type = gpu::VertexElementType::FLOAT2},
					{.binding = 0, .offset = offsetof(ImDrawVert, uv), .type = gpu::VertexElementType::FLOAT2},
					{.binding = 0, .offset = offsetof(ImDrawVert, col), .type = gpu::VertexElementType::UINT}
				},
				.viewport = {
			...
			for (auto n = 0; n < draw_data.CmdListsCount; n++) {
				...
				for (auto cmd_i = 0; cmd_i < cmd_list.CmdBuffer.Size; cmd_i++) {
					...
					if (cmd.UserCallback != NULL) {
						...
						if (clip_rect.x < viewport.x && clip_rect.y < viewport.y && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
						{
							if (clip_rect.x < 0.0f)
								clip_rect.x = 0.0f;
							if (clip_rect.y < 0.0f)
							...
							const PushConstant push_constant = {
								.transform_descriptor_id = gpu_system_->get_ssbo_descriptor_id(registry.get_buffer(parameter.transform_buffer)),
							...
							const Command command = {
								...
								.index_type = sizeof(ImDrawIdx) == 2 ? gpu::IndexType::UINT16 : gpu::IndexType::UINT32,
