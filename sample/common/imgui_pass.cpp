			...
			for (auto n = 0; n < draw_data.CmdListsCount; n++) {
				...
				for (auto cmd_i = 0; cmd_i < cmd_list.CmdBuffer.Size; cmd_i++) {
					...
					if (cmd.UserCallback != NULL) {
						...
						if (clip_rect.x < static_cast<float>(viewport.x) && 
							clip_rect.y < static_cast<float>(viewport.y) && 
							clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
						{
                            ...
                            static constexpr gpu::IndexType INDEX_TYPE = sizeof(ImDrawIdx) == 2 ? gpu::IndexType::UINT16 : gpu::IndexType::UINT32;
