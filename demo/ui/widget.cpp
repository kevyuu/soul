...
namespace Demo {
	namespace UI {
		...
		struct PiePopupContext {
			ImVec2 center;
			int lastFrame;
		};

		static PiePopupContext s_piePopupContext;
		...
		int PiePopupSelectMenu(const char* popup_id, const char** items, int items_count, int triggerKey)
		{
			...
			{
				if (s_piePopupContext.lastFrame < ImGui::GetFrameCount() - 1) {
					s_piePopupContext.center = ImGui::GetIO().MousePos;
				}
				s_piePopupContext.lastFrame = ImGui::GetFrameCount();
				ImVec2 pieCenter = s_piePopupContext.center;

				const ImVec2 drag_delta = ImVec2(ImGui::GetIO().MousePos.x - pieCenter.x, ImGui::GetIO().MousePos.y - pieCenter.y);
				...
				draw_list->PathArcTo(pieCenter, (RADIUS_MIN + RADIUS_MAX) * 0.5f, 0.0f, IM_PI * 2.0f * 0.99f, 32);   // FIXME: 0.99f look like full arc with closed thick stroke has a bug now
				draw_list->PathStroke(ImColor(colors[ImGuiCol_WindowBg]), true, RADIUS_MAX - RADIUS_MIN);
				...
				for (int item_n = 0; item_n < items_count; item_n++)
				{
					...
					draw_list->PathArcTo(pieCenter, RADIUS_MAX - style.ItemInnerSpacing.x, item_ang_min, item_ang_max, arc_segments);
					draw_list->PathArcTo(pieCenter, RADIUS_MIN + style.ItemInnerSpacing.x, item_ang_max, item_ang_min, arc_segments);
					//draw_list->PathFill(window->Color(hovered ? ImGuiCol_HeaderHovered : ImGuiCol_FrameBg));
					draw_list->PathFillConvex(ImColor(hovered ? colors[ImGuiCol_Button] : colors[ImGuiCol_ButtonHovered]));

					ImVec2 text_size = ImGui::CalcTextSize(item_label, NULL, true);
					ImVec2 text_pos = ImVec2(
						pieCenter.x + cosf((item_ang_min + item_ang_max) * 0.5f) * (RADIUS_MIN + RADIUS_MAX) * 0.5f - text_size.x * 0.5f,
						pieCenter.y + sinf((item_ang_min + item_ang_max) * 0.5f) * (RADIUS_MIN + RADIUS_MAX) * 0.5f - text_size.y * 0.5f);
					draw_list->AddText(text_pos, ImColor(colors[ImGuiCol_Text]), item_label);
				...
				if (ImGui::IsKeyReleased(triggerKey))
				{
