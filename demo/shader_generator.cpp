...
namespace Demo {
	...
	static void _GenerateProlog(Soul::StringBuilder& stringBuilder, ShaderType shaderType) {
		const char* defaultPrecisionQual = _GetPrecisionQualifier(_GetDefaultPrecision(shaderType));
		stringBuilder
			.append("#version 450 core\n\n")
			.append("#extension GL_GOOGLE_cpp_style_line_directive : enable\n\n")
			.append("#define TARGET_VULKAN_ENVIRONMENT\n")
			.append("#define FILAMENT_VULKAN_SEMANTICS\n")
			.append("#define FILAMENT_HAS_FEATURE_TEXTURE_GATHER\n")
			.append("precision %s float;\n", defaultPrecisionQual)
			.append("precision %s int;\n\n", defaultPrecisionQual);
	}

	static void _GenerateDefine(Soul::StringBuilder& stringBuilder, const ShaderDefine& shaderDefine) {
		switch (shaderDefine.type) {
			case ShaderDefineType::BOOL: {
				if (shaderDefine.boolean) stringBuilder.append("#define %s\n", shaderDefine.name);
				break;
			}
			case ShaderDefineType::STRING: {
				stringBuilder.append("#define %s %s\n", shaderDefine.name, shaderDefine.string);
				break;
			}
			case ShaderDefineType::INTEGER: {
				stringBuilder.append("#define %s %" PRIu64 "\n", shaderDefine.name, shaderDefine.integer);
				break;
			}
			default: {
				SOUL_NOT_IMPLEMENTED();
	...
	static void _GenerateShaderInput(Soul::StringBuilder& stringBuilder, const ShaderInput& shaderInput, uint32 location) {
		...
		stringBuilder.append("layout(location = %" PRIu32 ") in %s %s %s", location, precisionQual, varTypeName, shaderInput.name);
		if (shaderInput.count != 1) {
			stringBuilder.append("[%" PRIu32 "]", shaderInput.count);
		}
		stringBuilder.append(";\n");
	}

	static void _GenerateShaderOutput(Soul::StringBuilder& stringBuilder, const ShaderOutput& shaderOutput, uint32 location) {
		...
		stringBuilder.append("layout(location = %" PRIu32 ") out %s %s %s", location, precisionQual, varTypeName, shaderOutput.name);
		if (shaderOutput.count != 1) {
			stringBuilder.append("[%" PRIu32 "]", shaderOutput.count);
		}
		stringBuilder.append(";\n");
	}

	static void _GenerateUniform(Soul::StringBuilder& stringBuilder, const ShaderUniform& shaderUniform) {
		stringBuilder.append("layout(set = %d, binding = %d, std140) uniform %s {\n", shaderUniform.binding, shaderUniform.set, shaderUniform.typeName);
		for (uint64 memberIndex = 0; memberIndex < shaderUniform.memberCount; memberIndex++) {
			...
			stringBuilder.append("     %s %s %s", precision, typeName, member.name);
			if (member.count > 1) {
				stringBuilder.append("[%" PRIu32 "]", member.count);
			}
			stringBuilder.append(";\n");
		}
	...
	static void _GenerateSampler(Soul::StringBuilder& stringBuilder, const ShaderSampler& shaderSampler) {
		...
		stringBuilder.append("layout(set = %d, binding = %d) uniform %s %s %s;\n",
	...
	void ShaderGenerator::addShaderTemplates(const char* groupName, const char* path) {
		for (const auto& entry : std::filesystem::directory_iterator(path)) {
			...
			int templateKeyLength = strlen(groupName) + strlen("::") + strlen(filename.c_str());

			char* templateKey = (char*) scopeAllocator.allocate((templateKeyLength + 1) * sizeof(char), alignof(char), "").addr;
			sprintf(templateKey, "%s::%s", groupName, filename.c_str());
			uint64 templateKeyHash = _GetHash(templateKey, templateKeyLength);
	...
	Soul::GPU::ShaderID ShaderGenerator::createShader(const ShaderDesc& shaderDesc) const {
		...
		Soul::StringBuilder stringBuilder(_allocator, 10000);
