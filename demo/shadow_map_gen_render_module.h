...
public:
	...
	Parameter addPass(GPU::System* system, GPU::RenderGraph* renderGraph, const Parameter& data, const Scene& scene) {
		Parameter passData = data;
		for (int i = 0; i < 4; i++)
		{
			passData = renderGraph->addGraphicPass<Parameter>(
				"Shadow map gen pass",
				[this, &passData, &scene, i]
				(GPU::GraphicNodeBuilder* builder, Parameter* data) -> void
				{
					...
					uint16 resolution = DirectionalLight::SHADOW_MAP_RESOLUTION;
				...
				(GPU::RenderGraphRegistry* registry, const Parameter& data, GPU::CommandBucket* commandBucket) -> void
				{
					...
					for (int j = 0; j < scene.meshEntities.size(); j++)
					{
						...
						const MeshEntity& meshEntity = scene.meshEntities[j];
