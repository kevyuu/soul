...
namespace Demo {
    ...
    struct ShaderInput {
        ...
        ShaderInput() {}
        ShaderInput(const char* name, ShaderVarType type, ShaderPrecision precision = ShaderPrecision::DEFAULT, uint32 count = 1) :
    ...
    struct ShaderOutput {
        ...
        ShaderPrecision precision;
        ShaderOutput() {}
        ShaderOutput(const char* name, ShaderVarType type, ShaderPrecision precision = ShaderPrecision::DEFAULT, uint32 count = 1) :
            name(name), varType(type), precision(precision), count(count) {}
    };
    ...
    struct ShaderDefine {
        ...
        ShaderDefine(const char* name) : name(name), boolean(true), type(ShaderDefineType::BOOL) {}
        ShaderDefine(const char* name, uint64 integer) : name(name), integer(integer), type(ShaderDefineType::INTEGER) {}
        ShaderDefine(const char* name, const char* str) : name(name), string(str), type(ShaderDefineType::STRING) {}
    };
