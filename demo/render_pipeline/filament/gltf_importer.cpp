...
namespace soul_fila
{
    ...
    static bool get_vertex_attr_type(const cgltf_attribute_type src_type, uint32 index, const UvMap& uvmap, soul_fila::VertexAttribute* attr_type, bool* hasUv0) {
        using namespace soul_fila;
        switch (src_type) {
        case cgltf_attribute_type_position:
            *attr_type = VertexAttribute::POSITION;
            return true;
        ...
        case cgltf_attribute_type_color:
            *attr_type = VertexAttribute::COLOR;
            return true;
        case cgltf_attribute_type_joints:
            *attr_type = VertexAttribute::BONE_INDICES;
            return true;
        case cgltf_attribute_type_weights:
            *attr_type = VertexAttribute::BONE_WEIGHTS;
            return true;
...
// Returns false if the given component type is invalid.
    static bool get_element_type(const cgltf_type type, const cgltf_component_type ctype,
    ...
    static soul::GLSLMat3f matrix_from_uv_transform(const cgltf_texture_transform& uvt) {
        ...
        return soul::GLSLMat3f(mat_transform);
    }
    ...
    static void constrain_gpu_program_key(soul_fila::GPUProgramKey* key, UvMap* uvmap) {
        ...
        auto update_key_and_map = [&retval, &index](bool has_texture, const uint8 uv_index_key)->std::pair<bool,UvSet>
        {
        ...
        std::tie(has_texture, uv_index) = update_key_and_map(key->hasTransmissionTexture, key->transmissionUV);
        key->hasTransmissionTexture = has_texture;
        key->transmissionUV = uv_index;

        std::tie(has_texture, uv_index) = update_key_and_map(key->hasClearCoatTexture, key->clearCoatUV);
        key->hasClearCoatTexture = has_texture;
        key->clearCoatUV = uv_index;

        std::tie(has_texture, uv_index) = update_key_and_map(key->hasClearCoatRoughnessTexture, key->clearCoatRoughnessUV);
        key->hasClearCoatRoughnessTexture = has_texture;
        key->clearCoatRoughnessUV = uv_index;

        std::tie(has_texture, uv_index) = update_key_and_map(key->hasClearCoatNormalTexture, key->clearCoatNormalUV);
        key->hasClearCoatNormalTexture = has_texture;
        key->clearCoatNormalUV = uv_index;

        std::tie(has_texture, uv_index) = update_key_and_map(key->hasSheenColorTexture, key->sheenColorUV);
        key->hasSheenColorTexture = has_texture;
        key->sheenColorUV = uv_index;

        std::tie(has_texture, uv_index) = update_key_and_map(key->hasSheenRoughnessTexture, key->sheenRoughnessUV);
        key->hasSheenRoughnessTexture = has_texture;
        key->sheenRoughnessUV = uv_index;

        std::tie(has_texture, uv_index) = update_key_and_map(key->hasVolumeThicknessTexture, key->volumeThicknessUV);
    ...
    static bool get_topology(const cgltf_primitive_type in,
    ...
    static soul::gpu::BufferID create_index_buffer(soul::gpu::System& gpu_system, const cgltf_accessor& indices) {
        soul::runtime::ScopeAllocator<> scope_allocator("Create Index Buffer");

        const auto buffer_data_raw = soul::cast<const uint8*>(indices.buffer_view->buffer->data) + compute_binding_offset(indices);
        ...
        const soul::gpu::BufferID buffer_id = gpu_system.create_buffer(index_buffer_desc, indexes.data());
        gpu_system.finalize_buffer(buffer_id);
        return buffer_id;
    }

    static void add_attribute_to_primitive(Primitive* primitive, 
    ...
    AttributeBuffer get_attribute_buffer(soul::memory::Allocator& allocator, const cgltf_attribute& src_attribute, const cgltf_accessor& accessor)
    {
        if (accessor.is_sparse || src_attribute.type == cgltf_attribute_type_tangent ||
    ...
    IndexData create_index_buffer(soul::memory::Allocator& allocator, soul::gpu::System& gpu_system, const cgltf_primitive& src_primitive)
    {
        ...
        if (src_primitive.indices) {
            ...
            return { soul::cast<Vec3ui32*>(indexes), src_indices.count / 3, index_buffer};
        ...
        if (src_primitive.attributes_count > 0) {
            ...
            return { soul::cast<Vec3ui32*>(indexes), index_buffer_desc.count / 3, index_buffer };
    ...
    static AttributeData add_attributes_to_primitive(
        soul::memory::Allocator& allocator, soul::gpu::System& gpu_system, Primitive& dst_primitive, 
        const cgltf_primitive& src_primitive, const UvMap& uvmap, const IndexData& index_data)
    {
        ...
        for (cgltf_size attr_index = 0; attr_index < src_primitive.attributes_count; attr_index++) {
            ...
            if (src_attribute.type == cgltf_attribute_type_tangent) {
                SOUL_ASSERT(0, sizeof(Vec4f) == attribute_buffer.stride, "");
                attribute_data.tangents = soul::cast<Vec4f*>(attribute_buffer.data);
                continue;
            }
            if (src_attribute.type == cgltf_attribute_type_normal) {
                SOUL_ASSERT(0, sizeof(Vec3f) == attribute_buffer.stride, "");
                attribute_data.normals = soul::cast<Vec3f*>(attribute_buffer.data);
                continue;
            }
            ...
            if (src_attribute.type == cgltf_attribute_type_texcoord && src_attribute.index == 0) {
                ...
                attribute_data.uvs = soul::cast<Vec2f*>(generated);
            }

            if (src_attribute.type == cgltf_attribute_type_position) {
                SOUL_ASSERT(0, sizeof(Vec3f) == attribute_buffer.stride, "");
                attribute_data.positions = soul::cast<Vec3f*>(attribute_buffer.data);
                dst_primitive.aabb = aabbCombine(dst_primitive.aabb, AABB(Vec3f(accessor.min), Vec3f(accessor.max)));
            }

            VertexAttribute attr_type;
            const bool attr_supported = get_vertex_attr_type(src_attribute.type, src_attribute.index, uvmap, &attr_type, &has_uv0);
            if (!attr_supported)
                continue;
            ...
            const soul_size attribute_data_size = attribute_buffer.type_size * attribute_buffer.data_count;
            void* attribute_gpu_data = allocator.allocate(attribute_data_size, attribute_buffer.type_alignment);
            for (soul_size attribute_idx = 0; attribute_idx < attribute_buffer.data_count; attribute_idx++)
            {
                const uint64 offset = attribute_idx * attribute_buffer.stride;
                memcpy(soul::cast<byte*>(attribute_gpu_data) + (attribute_idx * attribute_buffer.type_size), attribute_buffer.data + offset, attribute_buffer.type_size);
            }

            const gpu::BufferID attribute_gpu_buffer = gpu_system.create_buffer(gpu_desc, attribute_gpu_data);
            ...
            if (attr_type == VertexAttribute::BONE_INDICES) flags |= gpu::VERTEX_ELEMENT_INTEGER_TARGET;

            add_attribute_to_primitive(&dst_primitive, attr_type, attribute_gpu_buffer, actual, flags, soul::cast<uint8>(attribute_buffer.type_size));
        }

        if (attribute_data.normals != nullptr || src_primitive.material && !src_primitive.material->unlit) {
        	...
        	if (ComputeTangentFrame(TangentFrameComputeInput(
                vertex_count, 
                attribute_data.normals, attribute_data.tangents, attribute_data.uvs, attribute_data.positions, 
                index_data.triangles32, index_data.triangleCount), qtangents)) {

                attribute_data.qtangents = allocator.create_raw_array<Vec4i16>(vertex_count);
                std::transform(qtangents, qtangents + vertex_count, attribute_data.qtangents, [](const Quaternionf& qtangent)
                {
                ...
                const soul::gpu::BufferID qtangents_gpu_buffer = gpu_system.create_buffer(qtangents_buffer_desc, attribute_data.qtangents);
                gpu_system.finalize_buffer(qtangents_gpu_buffer);
                add_attribute_to_primitive(&dst_primitive, VertexAttribute::QTANGENTS, qtangents_gpu_buffer, gpu::VertexElementType::SHORT4, gpu::VERTEX_ELEMENT_NORMALIZED, sizeof(Vec4i16));
            }
        }
    ...
    static void add_morph_attributes_to_primitive(soul::memory::Allocator& allocator, soul::gpu::System& gpu_system, 
        AttributeData& attribute_data, Primitive& dst_primitive,
        const cgltf_primitive& src_primitive, const UvMap& uvmap, const IndexData& index_data)
    {
        cgltf_size targets_count = src_primitive.targets_count;
        if (targets_count > MAX_MORPH_TARGETS) SOUL_LOG_WARN("Cannot load all the morph targets. num target = %d, max target = %d", targets_count, MAX_MORPH_TARGETS);
        ...
        for (cgltf_size target_index = 0; target_index < targets_count; target_index++) {
            ...
            auto get_morph_target_type = [](cgltf_attribute_type atype, MorphTargetType* target_type) -> bool {
                switch (atype) {
                case cgltf_attribute_type_position:
                    *target_type = MorphTargetType::POSITION;
                    return true;
                case cgltf_attribute_type_tangent:
                    *target_type = MorphTargetType::TANGENT;
                    return true;
                case cgltf_attribute_type_normal:
                    *target_type = MorphTargetType::NORMAL;
                    return true;
                case cgltf_attribute_type_color:
                case cgltf_attribute_type_weights:
                case cgltf_attribute_type_joints:
            ...
            for (cgltf_size attribute_index = 0; attribute_index < morph_target.attributes_count; attribute_index++) {
                ...
                const uint16 attribute_type_size = soul::cast<uint8>(cgltf_num_components(accessor.type) * sizeof(float));
                ...
                if (src_attribute.type == cgltf_attribute_type_position) {
                    ...
                    const soul_size attribute_gpu_data_size = attribute_buffer.data_count * attribute_type_size;
                    void* attribute_gpu_data = allocator.allocate(attribute_gpu_data_size, attribute_buffer.type_alignment);
                    for (soul_size attribute_idx = 0; attribute_idx < attribute_buffer.data_count; attribute_idx++)
                    {
                        const uint64 offset = attribute_idx * attribute_buffer.stride;
                        memcpy(soul::cast<byte*>(attribute_gpu_data) + (attribute_idx * attribute_type_size), attribute_buffer.data + offset, attribute_type_size);
                    }
                    gpu::BufferID attribute_gpu_buffer = gpu_system.create_buffer(gpu_desc, attribute_gpu_data);
                    gpu_system.finalize_buffer(attribute_gpu_buffer);
            ...
            if (morph_target_attributes[MorphTargetType::NORMAL]) {
                if (attribute_data.normals) {
                    ...
                    auto normal_target = soul::cast<const Vec3f*>(morph_target_attributes[MorphTargetType::NORMAL]);
                    ...
                    auto tangent_target = soul::cast<const Vec3f*>(morph_target_attributes[MorphTargetType::TANGENT]);
                    ...
                    auto position_target = soul::cast<const Vec3f*>(morph_target_attributes[MorphTargetType::POSITION]);
            ...
            if (ComputeTangentFrame(TangentFrameComputeInput(
                ...
                const soul::gpu::BufferID qtangents_gpu_buffer = gpu_system.create_buffer(qtangents_buffer_desc, qtangents);
                gpu_system.finalize_buffer(qtangents_gpu_buffer);
                add_attribute_to_primitive(&dst_primitive, static_cast<VertexAttribute>(base_tangents_attr + target_index), qtangents_gpu_buffer, gpu::VertexElementType::SHORT4, gpu::VERTEX_ELEMENT_NORMALIZED, sizeof(Quaternionf));
            }
    ...
    GLTFImporter::GLTFImporter(const char* gltf_path, soul::gpu::System& gpu_system, GPUProgramRegistry& program_registry, Scene& scene) :
        gltf_path_(gltf_path),
        gpu_system_(gpu_system),
		program_registry_(program_registry), scene_(scene)
    {}
    ...
    void GLTFImporter::import()
	{
        ...
        scene_.create_meshes(asset_->meshes_count, 
            [this](const soul_size mesh_index, Mesh& dst_mesh)
            {
                ...
                for (cgltf_size primitive_index = 0; primitive_index < src_mesh.primitives_count; primitive_index++) {
                    ...
                    dst_primitive.materialID = create_material(src_primitive.material, has_vertex_color, uvmap);

                    const bool get_topology_success = get_topology(src_primitive.type, &dst_primitive.topology);
                    SOUL_ASSERT(0, get_topology_success, "");

                    const IndexData index_data = create_index_buffer(primitive_scope_allocator, gpu_system_, src_primitive);
                    dst_primitive.indexBuffer = index_data.gpu_handle;

                    AttributeData attribute_data = add_attributes_to_primitive(primitive_scope_allocator, gpu_system_, dst_primitive, src_primitive, uvmap, index_data);
                    add_morph_attributes_to_primitive(primitive_scope_allocator, gpu_system_, attribute_data, dst_primitive, src_primitive, uvmap, index_data);
        ...
        SOUL_LOG_INFO("Finish loading meshes and textures");
        ...
        SOUL_LOG_INFO("Finish importing gltf : %s", gltf_path_);
	}

    MaterialID GLTFImporter::create_material(const cgltf_material* src_material, const bool vertex_color, UvMap& uvmap)
	{
        ...
        const bool has_texture_transforms =
            ...
            src_material->normal_texture.has_transform ||
            src_material->occlusion_texture.has_transform ||
            src_material->emissive_texture.has_transform ||
            cc_config.clearcoat_texture.has_transform ||
        ...
        program_key.clearCoatRoughnessUV = soul::cast<uint8>(cc_config.clearcoat_roughness_texture.texcoord);
        ...
        if (src_material->has_pbr_specular_glossiness) {
            ...
            if (sg_config.specular_glossiness_texture.texture) {
                ...
                program_key.brdf.specularGlossiness.UV = soul::cast<uint8>(metallic_roughness_texture.texcoord);
            }
        }
        else {
            program_key.brdf.metallicRoughness.hasTexture = !!metallic_roughness_texture.texture;
            program_key.brdf.metallicRoughness.UV = soul::cast<uint8>(metallic_roughness_texture.texcoord);
        }
        switch (src_material->alpha_mode) {
        case cgltf_alpha_mode_opaque:
            program_key.alphaMode = AlphaMode::OPAQUE;
            break;
        case cgltf_alpha_mode_mask:
            program_key.alphaMode = AlphaMode::MASK;
            break;
        case cgltf_alpha_mode_blend:
            program_key.alphaMode = AlphaMode::BLEND;
            break;
        }
        ...
        mat_buf.baseColorFactor = soul::Vec4f(mr_config.base_color_factor);
        mat_buf.emissiveFactor = soul::Vec3f(src_material->emissive_factor);
        ...
        if (program_key.useSpecularGlossiness) {
            mat_buf.baseColorFactor = soul::Vec4f(sg_config.diffuse_factor);
            mat_buf.specularFactor = soul::Vec3f(sg_config.specular_factor);
            mat_buf.roughnessFactor = mr_config.roughness_factor;
        }
        ...
        if (program_key.brdf.metallicRoughness.hasTexture) {
            ...
            mat_textures.metallicRoughnessTexture = create_texture(*metallic_roughness_texture.texture, srgb);
            if (program_key.hasTextureTransforms) {
        ...
        if (program_key.hasOcclusionTexture) {
            mat_textures.occlusionTexture = create_texture(*src_material->occlusion_texture.texture, false);
            if (program_key.hasTextureTransforms) {
                mat_buf.occlusionUvMatrix = matrix_from_uv_transform(src_material->occlusion_texture.transform);
            }
        ...
        if (program_key.hasEmissiveTexture) {
            ...
            if (program_key.hasTextureTransforms) {
                mat_buf.emissiveUvMatrix = matrix_from_uv_transform(src_material->emissive_texture.transform);
            }
        }
        ...
        if (program_key.hasClearCoat) {
            ...
            if (program_key.hasClearCoatTexture) {
                ...
                if (program_key.hasTextureTransforms) {
                    mat_buf.clearCoatUvMatrix = matrix_from_uv_transform(cc_config.clearcoat_texture.transform);
                }
            }

            if (program_key.hasClearCoatRoughnessTexture) {
                mat_textures.clearCoatRoughnessTexture = create_texture(*cc_config.clearcoat_roughness_texture.texture, false);
                if (program_key.hasTextureTransforms) {
                    mat_buf.clearCoatRoughnessMatrix = matrix_from_uv_transform(cc_config.clearcoat_roughness_texture.transform);
                }
            }

            if (program_key.hasClearCoatNormalTexture) {
                mat_textures.clearCoatNormalTexture = create_texture(*cc_config.clearcoat_normal_texture.texture, false);
                if (program_key.hasClearCoatNormalTexture) {
                    mat_buf.clearCoatNormalUvMatrix = matrix_from_uv_transform(cc_config.clearcoat_normal_texture.transform);
                }
        ...
        if (program_key.hasSheen) {
            mat_buf.sheenColorFactor = soul::Vec3f(sh_config.sheen_color_factor);
            mat_buf.sheenRoughnessFactor = sh_config.sheen_roughness_factor;

            if (program_key.hasSheenColorTexture) {
                mat_textures.sheenColorTexture = create_texture(*sh_config.sheen_color_texture.texture, true);
                if (program_key.hasTextureTransforms) {
                    mat_buf.sheenColorUvMatrix = matrix_from_uv_transform(sh_config.sheen_color_texture.transform);
                }
            }

            if (program_key.hasSheenRoughnessTexture) {
                mat_textures.sheenRoughnessTexture = create_texture(*sh_config.sheen_roughness_texture.texture, false);
                if (program_key.hasTextureTransforms) {
                    mat_buf.sheenRoughnessUvMatrix = matrix_from_uv_transform(sh_config.sheen_roughness_texture.transform);
                }
            }
        }
        ...
        if (program_key.hasVolume)
        {
            ...
            {
                mat_textures.volumeThicknessTexture = create_texture(*vl_config.thickness_texture.texture, false);
                if (program_key.hasTextureTransforms)
                {
                    mat_buf.volumeThicknessUvMatrix = matrix_from_uv_transform(vl_config.thickness_texture.transform);
                }
            }
        }
        ...
        if (program_key.hasTransmission) {
            ...
            if (program_key.hasTransmissionTexture) {
                mat_textures.transmissionTexture = create_texture(*tr_config.transmission_texture.texture, false);
                if (program_key.hasTextureTransforms) {
                    mat_buf.transmissionUvMatrix = matrix_from_uv_transform(tr_config.transmission_texture.transform);
                }
            }
        }
    ...
    void GLTFImporter::import_textures()
    {
        ...
        for (const auto tex_key : tex_key_list_)
        {
            ...
            runtime::create_and_run_task(create_gpu_textures_parent, [this, &src_texture, tex_id](runtime::TaskID)
            {
                Texture* texture = scene_.get_texture_ptr(tex_id);

                auto load_texels = [this](const cgltf_texture& src_texture) -> std::tuple<const byte*, Vec2ui32, uint32>
                {
                    ...
                    if (data) {
                        ...
                        const byte* texels = stbi_load_from_memory(sourceData, soul::cast<int>(total_size),
                            &width, &height, &comp, 4);
                        ...
                        auto uri_path = soul::cast<char*>(scope_allocator.allocate(strlen(gltf_path_) + GLTF_URI_MAX_LENGTH + 1, alignof(char)));
                ...
                constexpr soul::gpu::SamplerDesc default_sampler = soul::gpu::SamplerDesc::SameFilterWrap(soul::gpu::TextureFilter::LINEAR, soul::gpu::TextureWrap::REPEAT);
                const soul::gpu::SamplerDesc sampler_desc = src_texture.sampler != nullptr ? get_sampler_desc(*src_texture.sampler) : default_sampler;
    ...
    {
        std::for_each_n(asset_->nodes, asset_->nodes_count, [this](const cgltf_node& node) { create_entity(&node); });
        SOUL_LOG_INFO("Finish create entities");
    }
    ...
    EntityID GLTFImporter::create_entity(const cgltf_node* node)
    {
        ...
        const Mat4f world_transform = parent_transform.world * local_transform;
        ...
        scene_.add_component<TransformComponent>(entity, { local_transform, world_transform, parent, ENTITY_ID_NULL, next_entity, ENTITY_ID_NULL });
    ...
    void GLTFImporter::create_renderable(const EntityID entity, const cgltf_node* node) {
        ...
        soul::Vec4f morph_weights;
        if (num_morph_targets > 0) {
            std::copy_n(src_mesh.weights, std::min(MAX_MORPH_TARGETS, src_mesh.weights_count), morph_weights.mem);
            std::copy_n(node->weights, std::min(MAX_MORPH_TARGETS, node->weights_count), morph_weights.mem);
        }
        ...
        scene_.add_component<RenderComponent>(entity, { visibility, mesh_id, skin_id, morph_weights, uint8(0x1) });
    }
    ...
    void GLTFImporter::create_camera(const EntityID entity_id, const cgltf_node* node) {
        ...
        if (src_camera.type == cgltf_camera_type_perspective) {
            ...
            camera_component.setPerspectiveProjection(src_perspective.yfov, src_perspective.aspect_ratio, src_perspective.znear, far);
        }
            ...
            camera_component.setOrthoProjection(left, right, bottom, top, src_orthographic.znear, src_orthographic.zfar);
        }
        else {
            SOUL_NOT_IMPLEMENTED();
        }
    ...
    void GLTFImporter::create_light(const EntityID entity_id, const cgltf_node* node)
    {
        ...
        constexpr soul::Vec3f direction(0.0f, 0.0f, -1.0f);
        const soul::Vec3f color(light.color[0], light.color[1], light.color[2]);
        ...
        if (light_type.type == LightRadiationType::SPOT || light_type.type == LightRadiationType::FOCUSED_SPOT) {
            const float inner_clamped = std::min(std::abs(light.spot_inner_cone_angle), Fconst::PI_2);

        	float outer_clamped = std::min(std::abs(light.spot_outer_cone_angle), Fconst::PI_2);
        ...
        case LightRadiationType::POINT:
            luminous_intensity = luminous_power * Fconst::ONE_OVER_PI * 0.25f;
            break;
        ...
        case LightRadiationType::FOCUSED_SPOT: {
            ...
            luminous_power = luminous_intensity * (Fconst::TAU * (1.0f - cos_outer));
        ...
        scene_.add_component<LightComponent>(entity_id, { light_type, soul::Vec3f(0, 0, 0), direction, color, ShadowParams(), spot_params, 0.0f, 0.0f, 0.0f, luminous_intensity, falloff });
    }
    ...
    AnimationSampler create_animation_sampler(const cgltf_animation_sampler& src_sampler)
    {
        ...
        auto timeline_floats = soul::cast<const float*>(timeline_blob + timeline_accessor.offset +
        ...
        case cgltf_type_scalar:
            ...
            cgltf_accessor_unpack_floats(src_sampler.output, dst_sampler.values.data(), values_accessor.count);
            break;
        case cgltf_type_vec3:
            dst_sampler.values.resize(values_accessor.count * 3);
            cgltf_accessor_unpack_floats(src_sampler.output, dst_sampler.values.data(), values_accessor.count * 3);
            break;
        case cgltf_type_vec4:
            dst_sampler.values.resize(values_accessor.count * 4);
            cgltf_accessor_unpack_floats(src_sampler.output, dst_sampler.values.data(), values_accessor.count * 4);
            break;
        case cgltf_type_vec2:
        case cgltf_type_mat2:
        case cgltf_type_mat3:
        ...
        case cgltf_interpolation_type_step:
            dst_sampler.interpolation = AnimationSampler::STEP;
            break;
    ...
    void GLTFImporter::import_animations()
    {
        ...
        {
            Animation dst_anim;
            std::transform(src_animation.samplers, src_animation.samplers + src_animation.samplers_count, std::back_inserter(dst_anim.samplers), create_animation_sampler);

            auto create_animation_channel = [this, &dst_anim, &src_animation](const cgltf_animation_channel& src_channel) -> AnimationChannel
            {
            ...
            dst_anim.duration = 0;
            std::transform(src_animation.channels, src_animation.channels + src_animation.channels_count, std::back_inserter(dst_anim.channels), create_animation_channel);
            dst_anim.name = src_animation.name;
            return dst_anim;
        };

        scene_.create_animations_parallel(asset_->animations_count, [this, create_animation](soul_size anim_index, Animation& dst_animation)
        {
            dst_animation = create_animation(asset_->animations[anim_index]);
        });
        SOUL_LOG_INFO("Finish Loading animation");
    }

    void GLTFImporter::import_skins()
    {
        SOUL_PROFILE_ZONE();
        std::for_each_n(asset_->skins, asset_->skins_count, [this](const cgltf_skin& src_skin)
        {
            ...
            if (const cgltf_accessor* srcMatrices = src_skin.inverse_bind_matrices) {
                ...
                const auto src_buffer = soul::cast<void*>(bytes + srcMatrices->offset + srcMatrices->buffer_view->offset);
                memcpy(dst_matrices, src_buffer, src_skin.joints_count * sizeof(soul::Mat4f));
                for (soul::Mat4f& matrix : dst_skin.invBindMatrices) {
