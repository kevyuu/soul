...
namespace soul
{
  ...
  public:
    ...
    {
      other.visit(
        [this]<typeset VariantAltT>(VariantAltT& val) -> void
        {
    ...
    constexpr auto swap(Variant& other) noexcept
    {
      if (other.active_index_ == this->active_index_)
      {
        ...
        {
          other.visit(
            [&lhs = *this]<typeset VariantAltT>(VariantAltT& val) -> void
            {
              using std::swap;
      ...
      {
        this->visit(
          [&other]<typeset VariantAltT>(VariantAltT& val) -> void
          {
      ...
      {
        this->visit(
          [&rhs = other, &lhs = *this]<typeset ThisAltT>(ThisAltT& lhs_val) -> void
          {
            rhs.visit(
              [&rhs, &lhs, &lhs_val]<typeset OtherAltT>(OtherAltT& rhs_val) -> void
              {
  ...
  private:
    ...
    {
      other.visit(
        [this]<typeset VariantAltT>(const VariantAltT& val) -> void
        {
          static_assert(get_type_count_v<VariantAltT, Ts...> == 1, "Variant type is not found");
      ...
      requires(can_compare_equality_v<Ts> && ...)
    {
      return (lhs.active_index_ == rhs.active_index_) &&
             lhs.visit(
               [&rhs]<typeset VariantAltT>(const VariantAltT& lhs_val)
               {
