...
namespace Soul {
	...
	private:
		...
		inline void _copyValues(const HashMap<VALTYPE>& other) {
	...
	template <typename ValType>
	HashMap<ValType>::HashMap(const HashMap& other) :  _allocator(other._allocator) {
	...
	template <typename ValType>
	HashMap<ValType>& HashMap<ValType>::operator=(const HashMap& other) {
		if (this == &other) return *this;
		cleanup();
		new (this) HashMap<ValType>(other);
		return *this;
	}

	template<typename ValType>
	HashMap<ValType>::HashMap(HashMap&& other) noexcept {
	...
	template<typename ValType>
	HashMap<ValType>& HashMap<ValType>::operator=(HashMap&& other) noexcept {
	...
	template <typename ValType>
	void HashMap<ValType>::reserve(uint32 capacity) {
	...
	template <typename ValType>
	void HashMap<ValType>::add(uint64 key, const ValType& value) {
		ValType valueToInsert = value;
		add(key, std::move(valueToInsert));
	}

	template <typename ValType>
	void HashMap<ValType>::add(uint64 key, ValType&& value) {
		if (_size == _capacity) {
	...
	template <typename ValType>
	void HashMap<ValType>::remove(uint64 key) {
		uint32 index = _findIndex(key);
	...
	template <typename ValType>
	ValType& HashMap<ValType>::operator[](uint64 key) {
		uint32 index = _findIndex(key);
	...
	template <typename ValType>
	const ValType& HashMap<ValType>::operator[](uint64 key) const {
		uint32 index = _findIndex(key);
		if (_indexes[index].key == key && _indexes[index].dib != 0) {
			return _values[index];
		}
	...
	template <typename ValType>
	const ValType& HashMap<ValType>::get(uint64 key, const ValType& defaultValue) const {
		uint32 index = _findIndex(key);
		if (_indexes[index].key == key && _indexes[index].dib != 0) {
			return _values[index];
		} else {
