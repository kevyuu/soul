#pragma once

namespace soul
{
	struct IntrusiveListNode
    {
        IntrusiveListNode* prev = nullptr;
        IntrusiveListNode* next = nullptr;
	};

    template <typename T>
    concept intrusive_node_type = std::derived_from<T, IntrusiveListNode>;

	template<intrusive_node_type T, bool IsConst>
	class IntrusiveListIterator
	{
	public:

		using this_type = IntrusiveListIterator<T, IsConst>;
		using iterator = IntrusiveListIterator<T, false>;
	    using value_type = T;
		using node_type = T;
		using pointer = std::conditional_t<IsConst, const T*, T*>;
		using reference = std::conditional_t<IsConst, const T&, T&>;
		using iterator_category = std::bidirectional_iterator_tag;
        using difference_type = ptrdiff_t;

		pointer node;

		IntrusiveListIterator();
        explicit IntrusiveListIterator(pointer node);
        IntrusiveListIterator(const iterator& x);
        IntrusiveListIterator& operator=(const iterator& x);

		reference operator*() const;
        pointer operator->() const;

		IntrusiveListIterator& operator++();
        IntrusiveListIterator& operator--();

		IntrusiveListIterator operator++(int);
        IntrusiveListIterator operator--(int);
	};

    template <intrusive_node_type T, bool IsConst>
    IntrusiveListIterator<T, IsConst>::IntrusiveListIterator() : node(nullptr) {}

    template <intrusive_node_type T, bool IsConst>
    IntrusiveListIterator<T, IsConst>::IntrusiveListIterator(pointer node) : node(node) {}

    template <intrusive_node_type T, bool IsConst>
    IntrusiveListIterator<T, IsConst>::IntrusiveListIterator(const iterator& x) : node(x.node) {}

    template <intrusive_node_type T, bool IsConst>
    IntrusiveListIterator<T, IsConst>& 
	IntrusiveListIterator<T, IsConst>::operator=(const iterator& x)
    {
        node = x.node;
        return *this;
    }

    template <intrusive_node_type T, bool IsConst>
    typename IntrusiveListIterator<T, IsConst>::reference
    IntrusiveListIterator<T, IsConst>::operator*() const
    {
        return *node;
    }

    template <intrusive_node_type T, bool IsConst>
    typename IntrusiveListIterator<T, IsConst>::pointer
    IntrusiveListIterator<T, IsConst>::operator->() const
    {
        return node;
    }

    template <intrusive_node_type T, bool IsConst>
    IntrusiveListIterator<T, IsConst>& 
    IntrusiveListIterator<T, IsConst>::operator++()
    {
        node = static_cast<pointer>(node->next);
        return *this;
    }

    template <intrusive_node_type T, bool IsConst>
    IntrusiveListIterator<T, IsConst>& 
    IntrusiveListIterator<T, IsConst>::operator--()
    {
        node = static_cast<pointer>(node->prev);
        return *this;
    }

    template <intrusive_node_type T, bool IsConst>
    IntrusiveListIterator<T, IsConst>
    IntrusiveListIterator<T, IsConst>::operator++(int)
    {
        IntrusiveListIterator it(*this);
        node = static_cast<pointer>(node->next);
        return it;
    }

    template <intrusive_node_type T, bool IsConst>
    IntrusiveListIterator<T, IsConst>
    IntrusiveListIterator<T, IsConst>::operator--(int)
    {
        IntrusiveListIterator it(*this);
        node = static_cast<pointer>(node->prev);
        return it;
    }

    template <intrusive_node_type T, bool IsConst>
    inline bool operator==(const IntrusiveListIterator<T, IsConst>& a, 
        const IntrusiveListIterator<T, IsConst>& b)
    {
        return a.node == b.node;
    }

    template <intrusive_node_type T, bool IsConst, bool IsConst2>
    inline bool operator==(const IntrusiveListIterator<T, IsConst>& a, const IntrusiveListIterator<T, IsConst2>& b)
    {
        return a.node == b.node;
    }

    template <intrusive_node_type T>
	class IntrusiveList
	{
	public:
		using this_type = IntrusiveList<T>;
		using node_type = T;
		using value_type = T;
		using reference = T&;
		using const_reference = const T&;
		using pointer = T*;
		using const_pointer = const T*;
		using iterator = IntrusiveListIterator<T, false>;
		using const_iterator = IntrusiveListIterator<T, true>;
		using reverse_iterator = std::reverse_iterator<iterator>;
		using const_reverse_iterator = std::reverse_iterator<const_iterator>;

        IntrusiveList();
        IntrusiveList(const this_type& x) = delete;
        IntrusiveList(this_type&& x) = delete;

        this_type& operator=(const this_type& x) = delete;
        IntrusiveList& operator=(this_type&& x) = delete;

        ~IntrusiveList() = default;

        void swap(this_type& rhs) noexcept;
        friend void swap(this_type& a, this_type& b) noexcept { a.swap(b); }

		[[nodiscard]] iterator begin() noexcept;
        [[nodiscard]] const_iterator begin() const noexcept;
        [[nodiscard]] const_iterator cbegin() const noexcept;

		[[nodiscard]] iterator end() noexcept;
        [[nodiscard]] const_iterator end() const noexcept;
        [[nodiscard]] const_iterator cend() const noexcept;

		reverse_iterator rbegin() noexcept;
        [[nodiscard]] const_reverse_iterator rbegin() const noexcept;
        [[nodiscard]] const_reverse_iterator crbegin() const noexcept;

		reverse_iterator rend() noexcept;
        [[nodiscard]] const_reverse_iterator rend() const noexcept;
        [[nodiscard]] const_reverse_iterator crend() const noexcept;

        [[nodiscard]] soul_size size() const noexcept;
        [[nodiscard]] bool empty() const noexcept;

		[[nodiscard]] reference front();
        [[nodiscard]] const_reference front() const;
        [[nodiscard]] reference back();
        [[nodiscard]] const_reference back() const;

		void push_front(value_type& x);
        void push_back(value_type& x);
        void pop_front();
        void pop_back();

		bool contains(const value_type& x) const;

		[[nodiscard]] iterator locate(value_type& x);
        [[nodiscard]] const_iterator locate(const value_type& x) const;

		iterator insert(const_iterator pos, value_type& x);
        iterator erase(const_iterator pos);
        iterator erase(const_iterator pos, const_iterator last);

		static void remove(value_type& value);

        void clear() noexcept;

        void splice(const_iterator pos, value_type& value);
        void splice(const_iterator pos, this_type& list);
        void splice(const_iterator pos, this_type& list, const_iterator iterator);
        void splice(const_iterator pos, this_type& list, const_iterator first, const_iterator last);

	private:
        IntrusiveListNode anchor_;
	};

    template <intrusive_node_type T>
    IntrusiveList<T>::IntrusiveList() :
        anchor_({&anchor_, &anchor_}) {}

    template <intrusive_node_type T>
    void IntrusiveList<T>::swap(this_type& rhs) noexcept
    {
        IntrusiveListNode tmp(anchor_);
        anchor_ = rhs.anchor_;
        rhs.anchor_ = tmp;

        if (anchor_.next == &rhs.anchor_)
        {
            anchor_.next = &anchor_;
            anchor_.prev = &anchor_;
        }
        else
        {
            anchor_.next->prev = &anchor_;
            anchor_.prev->next = &anchor_;
        }

        if (rhs.anchor_.next == &anchor_)
        {
            rhs.anchor_.next = &rhs.anchor_;
            rhs.anchor_.prev = &rhs.anchor_;
        }
        else
        {
            rhs.anchor_.next->prev = &rhs.anchor_;
            rhs.anchor_.prev->next = &rhs.anchor_;
        }
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::iterator IntrusiveList<T>::begin() noexcept
    {
        return iterator(static_cast<T*>(anchor_.next));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::const_iterator IntrusiveList<T>::begin() const noexcept
    {
        return const_iterator(static_cast<T*>(anchor_.next));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::const_iterator IntrusiveList<T>::cbegin() const noexcept
    {
        return const_iterator(static_cast<T*>(anchor_.next));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::iterator IntrusiveList<T>::end() noexcept
    {
        return iterator(static_cast<T*>(&anchor_));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::const_iterator IntrusiveList<T>::end() const noexcept
    {
        return const_iterator(static_cast<const T*>(&anchor_));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::const_iterator IntrusiveList<T>::cend() const noexcept
    {
        return const_iterator(static_cast<const T*>(&anchor_));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::reverse_iterator IntrusiveList<T>::rbegin() noexcept
    {
        return reverse_iterator(iterator(static_cast<T*>(&anchor_)));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::const_reverse_iterator IntrusiveList<T>::rbegin() const noexcept
    {
        return const_reverse_iterator(const_iterator(static_cast<const T*>(&anchor_)));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::const_reverse_iterator IntrusiveList<T>::crbegin() const noexcept
    {
        return const_reverse_iterator(const_iterator(static_cast<const T*>(&anchor_)));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::reverse_iterator IntrusiveList<T>::rend() noexcept
    {
        return reverse_iterator(iterator(static_cast<T*>(anchor_.next)));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::const_reverse_iterator IntrusiveList<T>::rend() const noexcept
    {
        return const_reverse_iterator(const_iterator(static_cast<T*>(anchor_.next)));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::const_reverse_iterator IntrusiveList<T>::crend() const noexcept
    {
        return const_reverse_iterator(const_iterator(static_cast<T*>(anchor_.next)));
    }

    template <intrusive_node_type T>
    soul_size IntrusiveList<T>::size() const noexcept
    {
        soul_size size = 0;
        for (const IntrusiveListNode* node = anchor_.next; node != &anchor_; node = node->next)
        {
            size++;
        }
        return size;
    }

    template <intrusive_node_type T>
    bool IntrusiveList<T>::empty() const noexcept
    {
        return anchor_.next == &anchor_;
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::reference IntrusiveList<T>::front()
    {
        return *static_cast<T*>(anchor_.next);
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::const_reference IntrusiveList<T>::front() const
    {
        return *static_cast<T*>(anchor_.next);
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::reference IntrusiveList<T>::back()
    {
        return *static_cast<T*>(anchor_.prev);
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::const_reference IntrusiveList<T>::back() const
    {
        return *static_cast<T*>(anchor_.prev);
    }

    template <intrusive_node_type T>
    void IntrusiveList<T>::push_front(value_type& x)
    {
        x.next = anchor_.next;
        x.prev = &anchor_;

        anchor_.next->prev = &x;
        anchor_.next = &x;
    }

    template <intrusive_node_type T>
    void IntrusiveList<T>::push_back(value_type& x)
    {
        x.prev = anchor_.prev;
        x.next = &anchor_;

        anchor_.prev->next = &x;
        anchor_.prev = &x;
    }

    template <intrusive_node_type T>
    void IntrusiveList<T>::pop_front()
    {
        SOUL_ASSERT(0, !empty(), "List cannot be empty for pop front");
        erase(const_iterator(static_cast<const T*>(anchor_.next)));
    }

    template <intrusive_node_type T>
    void IntrusiveList<T>::pop_back()
    {
        SOUL_ASSERT(0, !empty(), "List cannot be empty for pop back");
        erase(const_iterator(static_cast<const T*>(anchor_.prev)));
    }

    template <intrusive_node_type T>
    bool IntrusiveList<T>::contains(const value_type& x) const
    {
        for (const IntrusiveListNode* node = anchor_.next; node != &anchor_; node = node->next)
        {
            if (node == &x)
                return true;
        }
        return false;
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::iterator IntrusiveList<T>::locate(value_type& x)
    {
        for (IntrusiveListNode* node = anchor_.next; node != &anchor_; node = node->next)
        {
            if (node == &x)
                return iterator(static_cast<T*>(node));
        }
        return iterator(static_cast<T*>(&anchor_));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::const_iterator IntrusiveList<T>::locate(const value_type& x) const
    {
        for (const IntrusiveListNode* node = anchor_.next; node != &anchor_; node = node->next)
        {
            if (node == &x)
                return const_iterator(static_cast<const T*>(node));
        }
        return const_iterator(static_cast<const T*>(&anchor_));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::iterator IntrusiveList<T>::insert(const_iterator pos, value_type& x)
    {
        IntrusiveListNode& next = *const_cast<node_type*>(pos.node);
        IntrusiveListNode& prev = *static_cast<node_type*>(next.prev);
        prev.next = &x;
        next.prev = &x;
        x.next = &next;
        x.prev = &prev;

        return iterator(&x);
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::iterator IntrusiveList<T>::erase(const_iterator pos)
    {
        IntrusiveListNode& next = *static_cast<node_type*>(pos.node->next);
        IntrusiveListNode& prev = *static_cast<node_type*>(pos.node->prev);
        prev.next = &next;
        next.prev = &prev;

        return iterator(static_cast<node_type*>(&next));
    }

    template <intrusive_node_type T>
    typename IntrusiveList<T>::iterator IntrusiveList<T>::erase(const_iterator pos, const_iterator last)
    {
        IntrusiveListNode& prev = *static_cast<node_type*>(pos.node->prev);
        IntrusiveListNode& next = *const_cast<node_type*>(last.node);

        prev.next = &next;
        next.prev = &prev;

        return iterator(const_cast<node_type*>(last.node));
    }

    template <intrusive_node_type T>
    void IntrusiveList<T>::remove(value_type& value)
    {
        IntrusiveListNode& prev = *value.prev;
        IntrusiveListNode& next = *value.next;

        prev.next = &next;
        next.prev = &prev;
    }

    template <intrusive_node_type T>
    void IntrusiveList<T>::clear() noexcept
    {
        anchor_.next = &anchor_;
        anchor_.prev = &anchor_;
    }

    template <intrusive_node_type T>
    void IntrusiveList<T>::splice(const_iterator pos, value_type& value)
    {
        if (pos.node != &value)
        {
            remove(value);
            insert(pos, value);
        }
    }

    template <intrusive_node_type T>
    // ReSharper disable once CppMemberFunctionMayBeStatic
    void IntrusiveList<T>::splice(const_iterator pos, this_type& list)
    {
        if (!list.empty())
        {
            IntrusiveListNode& new_next = *const_cast<node_type*>(pos.node);
            IntrusiveListNode& new_prev = *new_next.prev;
            IntrusiveListNode& list_head = *list.anchor_.next;
            IntrusiveListNode& list_tail = *list.anchor_.prev;

            new_prev.next = &list_head;
            new_next.prev = &list_tail;

            list_head.prev = &new_prev;
            list_tail.next = &new_next;

            list.anchor_.next = &list.anchor_;
            list.anchor_.prev = &list.anchor_;
        }
    }

    template <intrusive_node_type T>
    void IntrusiveList<T>::splice(const_iterator pos, this_type& list, const_iterator iterator)
    {
        if (pos != iterator)
        {
            list.erase(iterator);
            insert(pos, *(const_cast<T*>(iterator.node)));
        }
    }

    template <intrusive_node_type T>
    // ReSharper disable once CppMemberFunctionMayBeStatic
    void IntrusiveList<T>::splice(const_iterator pos, this_type& list, const_iterator first, const_iterator last)
    {
        if (first != last)
        {
            IntrusiveListNode& first_node = *const_cast<node_type*>(first.node);
            IntrusiveListNode& last_node = *(last.node->prev);

            first_node.prev->next = last_node.next;
            last_node.next->prev = first_node.prev;

            IntrusiveListNode& new_next = *const_cast<node_type*>(pos.node);
            IntrusiveListNode& new_prev = *new_next.prev;

            first_node.prev = &new_prev;
            last_node.next = &new_next;
            new_next.prev = &last_node;
            new_prev.next = &first_node;
        }
    }
}
