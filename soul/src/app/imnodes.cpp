...
namespace IMNODES_NAMESPACE
{
  namespace
  {
    ...
    inline bool RectangleOverlapsLineSegment(const ImRect& rect, const ImVec2& p1, const ImVec2& p2)
    {
      ...
      if (
        (p1.x < flip_rect.Min.x && p2.x < flip_rect.Min.x) ||
    ...
    // The draw list channels are structured as follows. First we have our base channel, the canvas
    // grid on which we render the grid lines in BeginNodeEditor(). The base channel is the reason
    // draw_list_submission_idx_to_background_channel_idx offsets the index by one. Each BeginNode()
    // call appends two new draw channels, for the node background and foreground. The node
    // foreground is the channel into which the node's ImGui content is rendered. Finally, in
    // EndNodeEditor() we append one last draw channel for rendering the selection box and the
    ...
    {
      // NOTE: don't use this function outside of EndNodeEditor. Using this before all nodes have
      // been added will screw up the node draw order.
    ...
    void DrawListActivateNodeBackground(const int node_idx)
    {
      ...
      // There is a discrepancy in the submitted node count and the rendered node count! Did you
    ...
    void DrawListSortChannelsByDepth(const ImVector<int>& node_idx_depth_order)
    {
      ...
      // TODO: this is an O(N^2) algorithm. It might be worthwhile revisiting this to see if the
    ...
    void ResolveOccludedPins(
      const ImNodesEditorContext& editor, ImVector<int>& occluded_pin_indices)
    {
      ...
      const ImObjectPool<ImLinkData>& links, const ImObjectPool<ImPinData>& pins)
    {
      ...
      // 1. The link is within hover distance to the mouse. The closest such link is selected as
      // being hovered over.
      ...
      for (int idx = 0; idx < links.Pool.Size; ++idx)
      {
        ...
        {
          if (
            GImNodes->HoveredPinIdx == link.StartPinIdx ||
  ...
  void EndNodeEditor()
  {
    ...
    if (!IsMiniMapHovered())
    {
      ...
      else if (GImNodes->LeftMouseClicked && GImNodes->HoveredNodeIdx.HasValue())
      {
        ...
        GImNodes->LeftMouseClicked || GImNodes->LeftMouseReleased || GImNodes->AltMouseClicked ||
