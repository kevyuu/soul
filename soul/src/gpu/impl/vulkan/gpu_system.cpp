...
#include "gpu/impl/vulkan/vk_check.h"
...
typedef vector <int, 4> vec4i32;

typedef vector <uint, 2> vec2u32;
typedef vector <uint, 3> vec3u32;
typedef vector <uint, 4> vec4u32;

typedef vector <int64_t, 2> vec2i64;
typedef vector <int64_t, 3> vec3i64;
typedef vector <int64_t, 4> vec4i64;

typedef vector <uint64_t, 2> vec2u64;
typedef vector <uint64_t, 3> vec3u64;
typedef vector <uint64_t, 4> vec4u64;

typedef vector <float, 2> vec2f32;
typedef vector <float, 3> vec3f32;
typedef vector <float, 4> vec4f32;

typedef vector <double, 2> vec2f64;
typedef vector <double, 3> vec3f64;
typedef vector <double, 4> vec4f64;

typedef matrix <float, 2, 2> mat2f32;
...
namespace soul::gpu
{
  ...
  auto System::init(const Config& config) -> void
  {
    ...
    config_ = config;
    runtime::ScopeAllocator<> scope_allocator("GPU::System::Init"_str);
    ...
    auto pick_physical_device = [](Database* db) -> FlagMap<QueueType, u32>
    {
      ...
      for (VkPhysicalDevice device : devices)
      {
        ...
        auto is_extension_available = [&available_extensions](const char* required_extension) -> b8
        {
          return std::ranges::any_of(
            available_extensions,
            [required_extension](const auto& properties)
            {
  ...
  auto System::init_frame_context(const Config& config) -> void
  {
    ...
    std::ranges::for_each(
      _db.frame_contexts,
      [this, config](FrameContext& frame_context)
      {
        frame_context.command_pools.init(_db.device, _db.queues, config.thread_count);
        frame_context.gpu_resource_initializer.init(
          _db.gpu_allocator, &frame_context.command_pools);
  ...
  auto System::get_gpu_properties() const -> const GPUProperties&
  {
  ...
  auto System::create_texture(String&& name, const TextureDesc& desc) -> TextureID
  {
    SOUL_PROFILE_ZONE();
    ...
    auto& texture         = _db.texture_pool.ref(texture_id);
    ...
    const auto tex_name = String::Format("{}({})", texture.name, _db.frame_counter);
  ...
  auto System::create_texture(
    String&& name, const TextureDesc& desc, const TextureLoadDesc& load_desc) -> TextureID
  {
    ...
    const TextureID texture_id = create_texture(std::move(name), new_desc);
    auto& texture              = texture_ref(texture_id);
  ...
  auto System::create_texture(String&& name, const TextureDesc& desc, const ClearValue clear_value)
    -> TextureID
  {
    TextureDesc new_desc = desc;
    new_desc.usage_flags |= {TextureUsage::TRANSFER_DST};
    ...
    TextureID texture_id = create_texture(std::move(name), new_desc);
    auto& texture        = texture_ref(texture_id);
  ...
  {
    get_frame_context().gpu_resource_finalizer.finalize(texture_ref(texture_id), usage_flags);
  }

  auto System::get_texture_mip_levels(const TextureID texture_id) const -> u32
  {
    return texture_cref(texture_id).desc.mip_levels;
  }
  ...
  auto System::destroy_texture_descriptor(TextureID texture_id) -> void
  {
    ...
    const auto& texture = texture_ref(texture_id);
  ...
  auto System::texture_ref(const TextureID texture_id) -> Texture& // NOLINT
  {
    return _db.texture_pool.ref(texture_id);
  }

  auto System::texture_cref(const TextureID texture_id) const -> const Texture&
  {
    return _db.texture_pool.cref(texture_id);
  }
    ...
    const TextureID texture_id, const Option<SubresourceIndex> subresource_index) -> TextureView
  {
    ...
    return texture_ref(texture_id).view;
  }

  auto System::texture_desc_cref(TextureID texture_id) const -> const TextureDesc&
  {
    return texture_cref(texture_id).desc;
  }
  ...
  auto System::create_blas(String&& name, const BlasDesc& desc, const BlasGroupID blas_group_id)
    -> BlasID
  {
    runtime::ScopeAllocator<> scope_allocator("create_blas(const BlasDesc&, BlasGroupID)"_str);
    const BufferDesc as_buffer_desc = {
      .size        = desc.size,
      .usage_flags = {BufferUsage::AS_STORAGE},
      .queue_flags = {QueueType::GRAPHIC, QueueType::COMPUTE},
    };
    const auto storage_buffer_id =
      create_buffer(String::Format("{}_storage_buffer", name), as_buffer_desc);

    const VkAccelerationStructureCreateInfoKHR create_info = {
      .sType  = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_CREATE_INFO_KHR,
      .buffer = buffer_ref(storage_buffer_id).vk_handle,
      .size   = desc.size,
    ...
    as_name.appendf("{}({})", name, _db.frame_counter);
    ...
    const auto blas_id = _db.blas_pool.create(Blas{
      .name        = std::move(name),
      .desc        = desc,
      .vk_handle   = vk_handle,
  ...
  auto System::destroy_blas(BlasID blas_id) -> void
  {
    ...
    const auto& blas    = blas_ref(blas_id);
    ...
    _db.blas_pool.destroy(blas_id);
  }

  auto System::blas_cref(BlasID blas_id) const -> const Blas&
  {
    return _db.blas_pool.cref(blas_id);
  }

  auto System::blas_ref(BlasID blas_id) -> Blas&
  {
    return _db.blas_pool.ref(blas_id);
  }
  ...
  auto System::get_gpu_address(BlasID blas_id) const -> GPUAddress
  {
    const VkAccelerationStructureDeviceAddressInfoKHR addr_info = {
      ...
      .accelerationStructure = blas_cref(blas_id).vk_handle};

    return GPUAddress(vkGetAccelerationStructureDeviceAddressKHR(_db.device, &addr_info));
  }

  auto System::create_blas_group(String&& name) -> BlasGroupID
  {
    auto id                          = _db.blas_group_pool.create();
    _db.blas_group_pool.ref(id).name = std::move(name);
  ...
  {
    SOUL_ASSERT_MAIN_THREAD();
    auto& blas_group    = blas_group_ref(blas_group_id);
    auto& frame_context = get_frame_context();
    for (const auto& blas_id : blas_group.blas_list)
    {
      auto& blas = blas_ref(blas_id);
      frame_context.garbages.as_vk_handles.push_back(blas.vk_handle);
      destroy_buffer(blas.buffer);
    }
    _db.blas_group_pool.destroy(blas_group_id);
  }

  auto System::blas_group_cref(BlasGroupID blas_group_id) const -> const BlasGroup&
  {
    return _db.blas_group_pool.cref(blas_group_id);
  }

  auto System::blas_group_ref(BlasGroupID blas_group_id) -> BlasGroup&
  {
    return _db.blas_group_pool.ref(blas_group_id);
  }
  ...
  auto System::create_tlas(String&& name, const TlasDesc& desc) -> TlasID
  {
    runtime::ScopeAllocator<> scope_allocator("gpu::System::create_tlas(const TlasDesc& desc)"_str);
    const BufferDesc as_buffer_desc = {
      .size        = desc.size,
      .usage_flags = {BufferUsage::AS_STORAGE},
      .queue_flags = {QueueType::GRAPHIC, QueueType::COMPUTE},
    ...
    const auto as_storage_buffer_id =
      create_buffer(String::Format("{}_storage_buffer", name), as_buffer_desc);

    const VkAccelerationStructureCreateInfoKHR create_info = {
      .sType  = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_CREATE_INFO_KHR,
      .buffer = buffer_ref(as_storage_buffer_id).vk_handle,
      .size   = desc.size,
  ...
  auto System::destroy_tlas(TlasID tlas_id) -> void
  {
    ...
    _db.tlas_pool.destroy(tlas_id);
  }

  auto System::tlas_cref(TlasID tlas_id) const -> const Tlas&
  {
    return _db.tlas_pool.cref(tlas_id);
  }

  auto System::tlas_ref(TlasID tlas_id) -> Tlas&
  {
    return _db.tlas_pool.ref(tlas_id);
  }

  auto System::create_buffer(String&& name, const BufferDesc& desc) -> BufferID
  {
    SOUL_ASSERT_MAIN_THREAD();
    return create_buffer(std::move(name), desc, false);
  }

  auto System::create_buffer(String&& name, const BufferDesc& desc, const void* data) -> BufferID
  {
    SOUL_PROFILE_ZONE();
    ...
    const BufferID buffer_id = create_buffer(std::move(name), new_desc);
    auto& buffer             = buffer_ref(buffer_id);
  ...
  auto System::create_transient_buffer(String&& name, const BufferDesc& desc) -> BufferID
  {
    const b8 use_linear_pool = desc.size < config_.transient_pool_size;
    const auto buffer_id     = create_buffer(std::move(name), desc, false);
  ...
  auto System::create_buffer(String&& name, const BufferDesc& desc, const b8 use_linear_pool)
    -> BufferID
  {
    ...
    Buffer& buffer       = buffer_ref(buffer_id);
    ...
    runtime::ScopeAllocator<> scope_allocator("Buffer name"_str);
    String buffer_name(&scope_allocator);
    buffer_name.appendf("{}(f{})", buffer.name, _db.frame_counter);
  ...
  auto System::get_gpu_allocator() -> VmaAllocator
  {
  ...
  auto System::acquire_swapchain() -> VkResult
  {
    ...
    auto& swapchain_texture              = texture_ref(swapchain_texture_id);
  ...
  auto System::buffer_ref(const BufferID buffer_id) -> Buffer&
  {
    SOUL_ASSERT(0, !buffer_id.is_null());
    return _db.buffer_pool.ref(buffer_id);
  }

  auto System::buffer_cref(const BufferID buffer_id) const -> const Buffer&
  {
    SOUL_ASSERT(0, !buffer_id.is_null());
    return _db.buffer_pool.cref(buffer_id);
  }

  auto System::get_gpu_address(BufferID buffer_id, usize offset) const -> GPUAddress
  {
    const VkBufferDeviceAddressInfo addr_info = {
      .sType  = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO,
      .buffer = buffer_cref(buffer_id).vk_handle,
    };

    return GPUAddress(vkGetBufferDeviceAddress(_db.device, &addr_info) + offset);
  }

  auto System::buffer_desc_cref(BufferID buffer_id) const -> const BufferDesc&
  {
    return buffer_cref(buffer_id).desc;
  }
    ...
    const TextureSampleCount sample_count) -> PipelineStateID
  {
    ...
    if (const auto id = _db.pipeline_state_cache.find(key); !id.is_null())
    {
    ...
    {
      runtime::ScopeAllocator<> scope_allocator("create_graphic_pipeline_state"_str);

      const Program& program = _db.program_pool.ref(desc.program_id);
  ...
  auto System::request_pipeline_state(const ComputePipelineStateDesc& desc) -> PipelineStateID
  {
    ...
    {
      const auto& program = _db.program_pool.ref(desc.program_id);
  ...
  {
    return _db.pipeline_state_cache.cref(pipeline_state_id);
  }
  ...
  auto System::create_program(const ProgramDesc& program_desc) -> Result<ProgramID, Error>
  {
    ...
    runtime::ScopeAllocator<> scope_allocator("gpu::System::create_program(const ProgramDesc&"_str);

    const auto program_id = ProgramID(_db.program_pool.create(Program()));
    Program& program      = _db.program_pool.ref(program_id);
  ...
  auto System::create_shader_table(String&& name, const ShaderTableDesc& shader_table_desc)
    -> ShaderTableID
  {
    runtime::ScopeAllocator<> scope_allocator(
      "gpu::System::create_shader_table(const ShaderTableDesc&)"_str);

    const auto& program = _db.program_pool.ref(shader_table_desc.program_id);
    const auto id       = ShaderTableID(_db.shader_table_pool.create());
    auto& shader_table  = shader_table_ref(id);
    shader_table.name   = std::move(name);
    ...
    pipeline_name.appendf("{}_pipeline({})", shader_table.name, _db.frame_counter);
    ...
    FlagMap<ShaderGroupKind, usize> group_counts = {};
    group_counts[ShaderGroupKind::RAYGEN]        = 1;
    group_counts[ShaderGroupKind::MISS]          = shader_table_desc.miss_groups.size();
    group_counts[ShaderGroupKind::HIT]           = shader_table_desc.hit_groups.size();
    group_counts[ShaderGroupKind::CALLABLE]      = 0;

    const auto group_names = FlagMap<ShaderGroupKind, const char*>::FromKeyValues({
      {ShaderGroupKind::RAYGEN, "raygen"},
      {ShaderGroupKind::MISS, "miss"},
      {ShaderGroupKind::HIT, "hit"},
      {ShaderGroupKind::CALLABLE, "callable"},
    });

    SOUL_ASSERT(
      0,
      group_counts[ShaderGroupKind::RAYGEN] == 1,
      "Only 1 raygen entry allowed since we use handle_size_aligned for the stride. Might need to "
      "use handleGroupBaseAlignment");
    usize current_storage_offset = 0;
    for (auto shader_group : FlagIter<ShaderGroupKind>())
    {
      const auto buffer_size = group_counts[shader_group] * handle_size_aligned;
      if (buffer_size == 0)
      {
        continue;
      }
      auto records = scope_allocator.allocate_array<u8>(buffer_size);
      for (usize entry_idx = 0; entry_idx < group_counts[shader_group]; entry_idx++)
      {
        memcpy(
          records + (entry_idx * handle_size_aligned),
          shader_handle_storage + current_storage_offset,
          handle_size);
        current_storage_offset += handle_size;
      }
      ...
      shader_table.buffers[shader_group] =
        create_buffer("Shader Table Buffer"_str, buffer_desc, records);
  ...
  {
    auto& shader_table  = shader_table_ref(shader_table_id);
    auto& frame_context = get_frame_context();
    ...
    _db.shader_table_pool.destroy(shader_table_id);
  }

  auto System::shader_table_cref(ShaderTableID shader_table_id) const -> const ShaderTable&
  {
    return _db.shader_table_pool.cref(shader_table_id);
  }

  auto System::shader_table_ref(ShaderTableID shader_table_id) -> ShaderTable&
  {
    return _db.shader_table_pool.ref(shader_table_id);
  }
  ...
  auto System::remove_from_blas_group(BlasID blas_id) -> void
  {
    SOUL_ASSERT_MAIN_THREAD();
    auto& blas       = blas_ref(blas_id);
    auto& blas_group = blas_group_ref(blas.group_data.group_id);
    auto& blas_list  = blas_group.blas_list;
    if (blas_list.back() != blas_id)
    {
      auto& last_blas                  = blas_ref(blas_list.back());
  ...
  {
    semaphore.visit(
      [this, wait_stages](auto& val)
      {
        wait(val, wait_stages);
      });
  }
  ...
  {
    return std::ranges::any_of(
      wait_timeline_values_,
      [](u64 timeline_value)
      {
        return timeline_value == 0;
      });
  }
  ...
  auto System::begin_frame() -> void
  {
...

    {
      ...
      {
        const auto& program = _db.program_pool.ref(program_id);
        for (const auto& shader : program.shaders)
        {
          vkDestroyShaderModule(_db.device, shader.vk_handle, nullptr);
        }
        _db.program_pool.destroy(program_id);
      }
    }

    {
      ...
      for (const auto texture_id : garbages.textures)
      {
        ...
        _db.texture_pool.destroy(texture_id);
      }
      garbages.textures.clear();
    }

    {
      ...
      for (const BufferID buffer_id : garbages.buffers)
      {
        ...
        const auto& buffer = buffer_ref(buffer_id);
        vmaDestroyBuffer(_db.gpu_allocator, buffer.vk_handle, buffer.allocation);
        _db.buffer_pool.destroy(buffer_id);
      }
      garbages.buffers.clear();
    }
  ...
  auto System::end_frame() -> void
  {
    ...
    auto& swapchain_texture = texture_ref(_db.swapchain.textures[swapchain_index]);
    {
  ...
  auto System::recreate_swapchain() -> void
  {
    ...
    std::ranges::transform(
      ...
      [this,
       image_sharing_mode = swapchain_info.imageSharingMode](VkImage image, VkImageView image_view)
      {
  ...
  auto System::get_ssbo_descriptor_id(BufferID buffer_id) const -> DescriptorID
  {
    ...
    return buffer_cref(buffer_id).storage_buffer_gpu_handle;
  }
  ...
  auto System::get_as_descriptor_id(TlasID tlas_id) const -> DescriptorID
  {
    ...
    return tlas_cref(tlas_id).descriptor_id;
  }
  ...
  auto GPUResourceFinalizer::finalize(Texture& texture, TextureUsageFlags usage_flags) -> void
  {
    ...
    static auto get_finalize_layout = [](TextureUsageFlags usage) -> VkImageLayout
    {
      ...
      usage.for_each(
        [&result](TextureUsage texture_usage)
        {
  ...
  {
    SOUL_ASSERT_MAIN_THREAD();
    runtime::ScopeAllocator scope_allocator("Resource Finalizer Flush"_str);
    ...
    for (auto queue_type : FlagIter<QueueType>())
    {
      ...
      sync_dst_queue.for_each(
        [&](const QueueType dst_queue_type)
        {
