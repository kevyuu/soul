...
namespace soul::gpu
{
	VkAccelerationStructureBuildGeometryInfoKHR compute_as_geometry_info(
		const BlasBuildDesc& build_desc, 
		const RTBuildMode build_mode,
		VkAccelerationStructureGeometryKHR* as_geometries)
	{
		std::ranges::transform(build_desc.geometry_descs, build_desc.geometry_descs + build_desc.geometry_count, as_geometries,
				...
				const auto geometry_data = [](const RTGeometryDesc& desc)-> VkAccelerationStructureGeometryDataKHR
				{
					if (desc.type == RTGeometryType::TRIANGLE)
					{
						...
						return {
							.triangles = {
								...
								.vertexData = {
									.deviceAddress = triangle_desc.vertex_data.id
								},
								.vertexStride = triangle_desc.vertex_stride,
								.maxVertex = triangle_desc.vertex_count,
								.indexType = vk_cast(triangle_desc.index_type),
								.indexData = {
									.deviceAddress = triangle_desc.index_data.id
								},
								.transformData = {
									.deviceAddress = triangle_desc.transform_data.id
								}
							}
						};
					}
					...
					return {
						.aabbs = {
							...
							.data = {
								.deviceAddress = aabb_desc.data.id
							},
    ...
    {
		std::transform(build_desc.geometry_descs, build_desc.geometry_descs + build_desc.geometry_count, max_primitives_counts,
