...
[[vk::push_constant]] TemporalDenoisePC push_constant;

groupshared u32 g_shadow_hit_masks[3][6];
...
void populate_cache(ComputeSV sv)
{
  ...
  if (sv.local_id.x < 3 && sv.local_id.y < 6)
  {
    ...
    g_shadow_hit_masks[sv.local_id.x][sv.local_id.y] = ray_query_result.Load(vec3i32(coord, 0)).x;
  }

  AllMemoryBarrierWithGroupSync();
}

f32 unpack_shadow_hit_value(vec2i32 coord, ComputeSV sv)
{
  ...
  const vec2i32 work_group_start_coord =
    ...
    vec2i32(TEMPORAL_DENOISE_WORK_GROUP_SIZE_X, TEMPORAL_DENOISE_WORK_GROUP_SIZE_Y);
...
f32 horizontal_neighborhood_mean(vec2i32 coord, ComputeSV sv)
{
  ...
  for (i32 x = -8; x <= 8; x++)
  {
    result += unpack_shadow_hit_value(vec2i32(coord.x + x, coord.y), sv);
  }
...
ReprojectResult reproject(
  ...
  Texture2D history_moments_length_texture)
{
  ...
  vec2f32 history_moments = vec2f32(0.0f, 0.0f);
  ...
  for (i32 sample_idx = 0; sample_idx < 4; sample_idx++)
  {
    ...
    vec4f32 sample_normal_roughness = prev_gbuffers.normal_roughness.Load(vec3i32(loc, 0));
    u32 sample_meshid               = prev_gbuffers.meshid.Load(vec3i32(loc, 0));
    f32 sample_depth                = prev_gbuffers.depth.Load(vec3i32(loc, 0)).r;

    vec3f32 sample_normal = sample_normal_roughness.xyz * 2.0 - 1.0;
    u32 history_mesh_id   = sample_meshid;

    vec3f32 sample_pos = world_position_from_depth(history_uv, sample_depth, prev_inv_proj_view_mat);
  ...
  if (valid)
  {
    ...
    for (i32 sample_idx = 0; sample_idx < 4; sample_idx++)
    {
      ...
      if (v[sample_idx])
      {
        ...
        history_moments += w[sample_idx] * history_moments_length_texture.Load(vec3i32(loc, 0)).rg;
        sumw += w[sample_idx];
      }
    }
    ...
    for (i32 yy = -radius; yy <= radius; yy++)
    {
      for (i32 xx = -radius; xx <= radius; xx++)
      {
              ...
              vec2i32(image_dim)))
        {
          ...
          history_moments += history_moments_length_texture.Load(vec3i32(p, 0)).rg;
          cnt += 1.0f;
        }
      }
    }
  ...
  if (valid)
  {
    history_length = history_moments_length_texture.Load(vec3i32(history_coord, 0)).b;
  } else
  {
    history_output  = 0.0f;
    history_moments = vec2f32(0.0f, 0.0f);
...
[numthreads(TEMPORAL_DENOISE_WORK_GROUP_SIZE_X, TEMPORAL_DENOISE_WORK_GROUP_SIZE_Y, 1)] //
  void
  cs_main(ComputeSV sv)
{
  ...
  RWTexture2D<vec2f32> output_val_texture =
    get_rw_texture_2d_vec2f32(push_constant.output_val_texture);
  RWTexture2D<vec4f32> output_moment_length_texture =
    get_rw_texture_2d_vec4f32(push_constant.output_moment_length_texture);
  ...
  Texture2D prev_moment_length_texture = get_texture_2d(push_constant.prev_moment_length_texture);
  ...
  f32 visibility                     = 0.0f;
  vec2f32 output_visibility_variance = vec2f32(0.0f, 0.0f);
  vec2f32 output_moments             = vec2f32(0.0f, 0.0f);
  f32 history_length                 = 0.0f;

  if (depth != 1.0f)
  {
    visibility = unpack_shadow_hit_value(texel_coord, sv);
    ...
    history_length         = min(32.0f, result.success ? result.history_length + 1.0f : 0.0f);
    f32 history_visibility = result.history_output;
    ...
    // the beginning.
    f32 alpha = result.success ? max(push_constant.alpha, 1.0f / (history_length + 1)) : 1.0;
    const f32 alpha_moments =
      result.success ? max(push_constant.moments_alpha, 1.0f / (history_length + 1)) : 1.0;
    ...
    output_moments = lerp(result.history_moments, output_moments, alpha_moments);

    output_visibility_variance.y =
      max(0.0f, output_moments.g - output_moments.r * output_moments.r);

    output_visibility_variance.x = lerp(history_visibility, visibility, alpha);
  }

  // Temporal integration
  output_moment_length_texture[texel_coord] = vec4f32(output_moments, history_length, unpack_shadow_hit_value(texel_coord, sv));
  output_val_texture[texel_coord]           = output_visibility_variance;
